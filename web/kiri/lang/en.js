...
self.kiri.lang['en'] = self.kiri.lang['en-us'] = {
  ...
  dv_time_l: [
    'modifier to print time estimates',
    'in the gcode export dialog.',
    'raise or lower value based on your',
    'predicted vs observed print times',
  ],
  ...
  dv_paps_l: [
    'minimum filament consumption',
    'between pings in millimeters',
    'recommended 500 - 1000',
    '0 = disable pings',
  ],
  dv_pafe_s: 'feed length',
  dv_pafe_l: [
    'feeder tube length in millimeters',
    'short=570, medium=800, long=1100',
  ],
  dv_papl_s: 'push length',
  dv_papl_l: [
    'distance between extruder gear',
    'and nozzle tip in millimeters',
  ],
  dv_paof_s: 'offset length',
  dv_paof_l: [
    'length to add or remove from',
    'the first segment in millimeters',
    'usually 0',
  ],
  ...
  op_shny_l:
    'choose a slice render method that is slower and uses specular highlighting. this makes it easier to see detail in some models at the expense of render speed',
  op_devl_s: 'developer',
  ...
  op_anta_l: [
    'smooth display. better but slower',
    'requires a page refresh or reload',
  ],
  ...
  op_anim_s: 'animate',
  op_anim_l: [
    'animation mesh density',
    'higher is more dense',
    'takes more memory',
    'and is slower',
  ],
  op_exgl_l: [
    'show Grid:Local devices in export dialog',
    'includes Grid:Bots and OctoPrint plugins',
  ],
  op_exgh_l: ['legacy Grid:Host local network support'],
  op_exop_l: [
    'legacy OctoPrint local network support',
    'Grid:Space OctoPrint plugin recommended',
  ],
  ...
  pt_deci_s: 'decimate',
  pt_deci_l: [
    'enable or disable point decimation',
    'during port import. for faster slicing',
    'and lower memory usage',
  ],
  pt_qual_s: 'detail',
  pt_qual_l: [
    'level of detail to retain during slicing operations. lower is faster and uses less memory. large scale models often do no benefit from higher detail. instead it my only result in memory and processing problems. reduce this value if you are getting out of memory errors.',
  ],
  pt_heal_s: 'heal mesh',
  pt_heal_l: [
    'attempt to heal non-manifold meshes during slicing. may increase slicing time. enables a warning when non-manifold geometries are detected.',
  ],
  pt_thrd_s: 'threaded',
  pt_thrd_l: [
    'use parallel processing when browser supports nested workers. may increase memory pressure on larger models. disable if you are getting out of memory errors',
  ],
  pt_assy_s: 'assembly',
  pt_assy_l: [
    'use web assembly when supported by browser. if you experience artifacts or memory errors, try disabling this.',
  ],
  ...
  sl_ltop_l: [
    'number of solid layers at the top of the print or on flat areas facing up',
  ],
  sl_lsld_s: 'solid layers',
  sl_lsld_l: [
    'solid fill areas computed',
    'from bridges and flat areas',
    'detected betwen sliced layers',
  ],
  sl_lbot_s: 'base layers',
  sl_lbot_l: [
    'number of solid layers at bottom of the print or under bridge spans',
  ],
  ...
  fi_grow_l: [
    'expand projected solids',
    'can help with solid areas',
    'over sparse infill areas',
    'units in millimeters',
  ],
  fi_over_s: 'fill overlap',
  fi_over_l: [
    'overlap with shell and other fill',
    'for improved extrusion bonding',
    'as fraction of nozzle diameter',
    '0.0 - 2.0',
  ],
  // fi_rate_s:      "print speed",
  fi_rate_l: [
    'extrusion speed for infill',
  ...
  fr_draf_l: [
    'extend skirt to height of part',
    'which acts as a shield against',
    'drafts and traps hot air',
  ],

  // FDM BELT ONLY
  fl_zoff_s: 'belt offset',
  fl_zoff_l: [
    'height offset from belt',
    'of lowest extrusion',
    'in millimeters',
  ],
  fl_flat_s: 'flatten',
  fl_flat_l: [
    'flatten to the belt any point',
    'height less than this value',
    'in millimeters',
  ],
  fl_brim_s: 'brim sides',
  fl_brim_l: [
    'add brim to part perimeter',
    'size is width in millimeters',
    '0 to disable',
  ],
  fl_brin_s: 'brim interior',
  fl_brin_l: [
    'add brim to part interior',
    'when interior voids exist',
    'size is width in millimeters',
    '0 to disable',
  ],
  fl_brmn_s: 'brim trigger',
  fl_brmn_l: [
    'add brims only when segment',
  ...
  fl_blmp_l: [
    'add a bump to the start of',
    'an anchor to help it peel',
    'off at the end of the belt',
    'in millimeters',
  ],
  ...
  sp_size_l: [
    'pillar width in millimeters',
    'smaller values exponentially',
    'increase detection times',
  ],
  ...
  sp_span_s: 'max span',
  sp_span_l: [
    'unsupported span that causes',
    'a new support to be generated',
    'in millimeters',
  ],
  sp_angl_s: 'max angle',
  sp_angl_l: [
    'max overhang angle before',
  ...
  cc_sovr_s: 'step over',
  cc_sovr_l: [
    'a fraction of tool diameter between 0 and 1. values greater than 0.49 may result in uncut areas or undesirable artifacts.',
  ],
  ...
  cc_offd_s: 'offset',
  cc_offd_l: [
    'overrides default offset distance (which is the tool radius) when set to any value but zero',
  ],
  cc_feed_s: 'feed rate',
  ...
  cc_sele_s: 'select',
  cc_sele_l: [
    'select lines for follow',
    'select loops for clear',
    'select surface for pocket',
  ],
  ...
  cc_lxyo_s: 'xy offset',
  cc_lxyo_l: [
    'xy offset from part boundary as a fraction of the tool diameter (0 - 1.0)',
  ],
  cc_lsto_s: 'stock',
  ...
  cr_lsto_l: [
    'horizontal offset from vertical faces',
    'stock to leave for finishing pass',
    'in workspace units',
  ],
  cr_lstz_s: 'leave stock z',
  cr_lstz_l: [
    'vertical offset for roughing passes in workspace units. best for roughing curved surfaces.',
  ],
  cr_ease_s: 'ease down',
  cr_ease_l: ['plunge cuts will', 'spiral down or ease', 'along a linear path'],
  cr_clst_s: 'clear stock',
  cr_clst_l: [
    'in indexed mode, clear entire stock area, not just to part periemter',
  ],
  cr_clrt_s: 'clear top',
  cr_clrt_l: [
    'run a clearing pass over',
  ...
  co_merg_l: [
    'merge overlapping lines to prevent overcutting into adjacent solids',
  ],
  co_dogb_s: 'dogbones',
  ...
  cf_angl_s: 'max angle',
  cf_angl_l: [
    'changes in z that result in angles greater than this are considered vertical and omitted from curves',
  ],
  ...
  cu_type_s: 'type',
  cu_type_l: [
    'clear = clear a closed-line pocket',
    'follow = tool tip follows line',
    'with offset option when closed',
  ],

  // CNC POCKET
  cp_xpnd_s: 'expand',
  cp_xpnd_l: [
    'grow (+) or shrink (-) selected areas',
    'contour uses entire selected area',
    'non-contour defaults to tool radius',
    'in workspace units',
  ],
  cp_smoo_s: 'smooth',
  cp_smoo_l: [
    'clean up jagged edges caused by angled walls. can speedup and improve pocketing for detailed geometries. recommend starting with values around 1 if slicing takes too long or the contours look jagged',
  ],
  cp_foll_s: 'selection',
  cp_foll_l: [
    'curvature selection limit in degrees. values closer to 0 will restrict selections to flatter surfaces',
  ],
  cp_refi_s: 'refine',
  cp_refi_l: [
    'number of refining passes to perform on contoured polylines. meant to address Z delta sawtoothing caused by faced geometries whereas smoothing handles XY. values > 10 work well for gently contoured geometries with significant Z movement.',
  ],
  cp_cont_s: 'contour',
  cp_cont_l: ['ignore interior voids and features'],
  cp_engr_s: 'engrave',
  cp_engr_l: [
    'sets up a single pass around the perimeter of the selected area. also useful for 3D laser marking',
  ],
  ...
  cd_dtru_l: [
    'additional drill depth below part bottom',
    'overrides global z thru',
    '0 to disable',
  ],
  cd_mark_s: 'marking',
  cd_mark_l: [
    'only mark holes, do not drill out',
    'the step down setting is used',
    'to determine how deep to mark',
  ],
  ...
  cl_adrp_s: 'repeating',
  cl_adrp_l:
    'apply a moduluse of Z height using the given band to make repeating patterns',
  cl_flat_s: 'flatten',
  ...
  cl_minp_l: [
    'power setting at the lowest part of the Z band. may be higher or lower than the other end of the Z band.',
  ],
  cl_maxp_s: 'z max power',
  cl_maxp_l: [
    'power setting at the highest part of the Z band. may be higher or lower than the other end of the Z band.',
  ],
  cl_minz_s: 'min z height',
  cl_minz_l: ['lowest Z band position'],
  cl_maxz_s: 'max z height',
  cl_maxz_l: [
    'highest Z band position. when set to 0, uses the highest Z point of the workspace',
  ],
  ...
  ls_offs_l: [
    'distance between the center of the cutting',
    'path and the edge of remaining material',
    'in millimeters',
  ],
  ...
  ou_mrgd_s: 'merged',
  ou_mrgd_l: [
    'merge all layers using',
    'color coding to denote',
    'stacking depth',
  ],
  ou_grpd_s: 'grouped',
  ou_grpd_l: [
    'retain each layer as',
    'a unified grouping',
  ...
  ou_lays_l: [
    'mark layers for stacking. the layer above will be output in a different color which the laser can mark with a lower power. turns on layer grouping.',
  ],
  ...
  ou_stak_s: '3d stack',
  ou_stak_l: [
    'output is a 3D stack of 2D paths rather than packing layers flat in 2D',
  ],
  ou_maxp_s: 'max power',
  ou_maxp_l: [
    'max power value in gcode. power % from settings will scale from 0 to this value',
  ],
  ou_inch_s: 'imperial',
  ...
  ou_fini_l: [
    'outermost shell speed',
    'bridge/flat solid layers',
    'set to 0 to use default',
    'millimeters / second',
  ],
  ou_move_s: 'move speed',
  ou_move_l: [
    'non-print move speed',
    'millimeters / second',
    '0 = enable G0 moves',
  ],
  ...
  ou_flat_s: 'flatness',
  ou_flat_l: [
    'delta z below which two adjacent points are considered co-linear. 0 = compute using tolerance',
  ],
  ou_redu_s: 'reduction',
  ou_redu_l: [
    'number of axis steps to skip for each unit of precision. higher precision may require reduction to produce reasonable file sizes. start at 0 and test in increments of 1.',
  ],
  ou_brdg_s: 'bridging',
  ou_brdg_l: ['flat distance between curved', 'areas to bridge with a cut'],
  ou_zanc_s: 'z anchor',
  ou_zanc_l: [
    'controls the position of the part',
  ...
  ou_ztop_s: 'z top',
  ou_ztop_l: [
    'offset from stock bottom',
    'to set start of cutting depth',
    'in workspace units',
    '* drill/contour ignore this *',
  ],
  ou_zbot_s: 'z bottom',
  ou_zbot_l: [
    'offset from stock bottom',
  ...
  ou_toin_s: 'tool init',
  ou_toin_l:
    'add tool change with first operation. when disabled, the gcode assumes the first tool is loaded and offset probed',
  ou_forz_s: 'force z max',
  ou_forz_l:
    'all moves between pockets will go to z max (above stock). this is usually required if you are not performing a roughing operation before pocketing operations.',
  ou_z1st_s: 'first z max',
  ou_z1st_l:
    'first move in the job will be to Z max (travel distance above stock)',
  ou_feng_s: 'engage factor',
  ou_feng_l:
    'speed scale (0.1-1) when endmill is fully engaged on a new cut. stepped over cuts when the endmill is less than fully engaged run at a factor of 1 relative to output speed. typically this relates to roughing or any operation with step over.',
  ...
  ad_abkl_l: [
    'for better flat surface finish',
  ...
  ad_lbir_l: [
    'always print shells touching the belt before any other shells. this is no longer a recommended setting based on extensive testing.',
  ],
  ad_altr_s: 'alternating',
  ad_altr_l: [
    'alternate shell winding order',
    'clockwise / counter-clockwise',
    'may reduce warping in thin areas',
    'and help with belt edge adhesion',
  ],
  ...
  ad_agap_l: [
    'route around gaps rather',
    'than perform a retract',
    'when possible',
  ],
  ...
  ad_purg_l: [
    'purge tower area in mm^2',
    'for multi extruder setups',
    '0 = disabled',
  ],
  ...
  cx_true_l: [
    'computationally correct shadow',
    'will be slower but produce',
    'better cuts for complex parts',
  ],
  ...
  ag_peel_l: [
    'starting at this belt z position',
    'periodically roll the print off and',
    'back on to the belt to unstick it',
    'and prevent rolling deflection',
  ],
