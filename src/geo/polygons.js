...
gapp.register('geo.polygons', [], (root, exports) => {
  ...
  function points(polys) {
    return polys.length
      ? polys.map((p) => p.deepLength).reduce((a, v) => a + v)
      : 0;
  }
      ...
      for (let i = 0; i < set.length; i++) {
        ...
        for (let j = i + 1; j < set.length; j++) {
          ...
          if (p1.overlaps(p0) && p1.intersects(p0)) {
            ...
            if (xor.length === 2) {
              let same0 =
                Math.abs(p0.area() - xor[0].area()) +
                  Math.abs(p1.area() - xor[1].area()) <
                0.01;
              let same1 =
                Math.abs(p1.area() - xor[0].area()) +
                  Math.abs(p0.area() - xor[1].area()) <
                0.01;
              if (same0 || same1) {
                continue;
              }
  ...
  function offset(polys, dist, opts = {}) {
    ...
    if (opts.wasm && geo.wasm) {
      try {
        polys = geo.wasm.js.offset(
          polys,
          offs,
          zed,
          clean ? config.clipperClean : 0,
          simple ? 1 : 0
        );
  ...
  function inset(polys, dist, count, z, wasm) {
    ...
    while (count-- > 0 && ref && ref.length) {
      ...
      let cref = cmp.length
        ? cmp.map((p) => p.areaDeep()).reduce((a, p) => a + p)
        : 0;
  ...
  function fillArea(polys, angle, spacing, output, minLen, maxLen) {
    ...
    let rayint = output || [],
      ...
      start =
        angle < 0
          ? { x: bounds.minx, y: bounds.miny, z: zpos }
          : { x: bounds.maxx, y: bounds.miny, z: zpos },
      clip = new Clipper(),
      tree = new PolyTree(),
      ...
      points.sort(function (p1, p2) {
        ...
        if (!(p1.del || p2.del) && p1.isNear(p2, merge_dist)) {
          ...
          if (line.length < 2) {
            ...
            p1.del = true;
            p2.del = true;
          } else if (!util.intersectRayLine(start, slope, line[0], line[1])) {
            /**
