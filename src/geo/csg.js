...
CSG.fromPositionArray = function(array) {
    let polys = [];
    for (let i=0, l=array.length; i<l; ) {
        polys.push(new Polygon([
            new Vertex(new Vector(
                array[i++],
                array[i++],
                array[i++],
            )),
...
CSG.computeFaceNormal = function(vA,vB,vC) {
    let cb = vC.clone().sub(vB);
    let ab = vA.clone().sub(vB);
    cb.cross(ab);
    return cb.normalize();
};
...
// zero normal for Vertices when none provided
let nnn = new Vector();
...
class Vertex {
    constructor(pos, normal, uv, color) {
        ...
        this.normal = new Vector().copy(normal || nnn);
...
// add to base objects
base.CSG = { Solid: CSG, Vector, Vertex, Plane, Node };
