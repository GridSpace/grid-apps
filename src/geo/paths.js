...
gapp.register('geo.paths', [], (root, exports) => {
  ...
  function poly2polyEmit(array, startPoint, emitter, opt = {}) {
    ...
    for (;;) {
      ...
      startPoint =
        emitter(found.poly, found.index, ++count, startPoint) || found.point;
    }
  ...
  function pointsToPath(points, offset, open, miter = 1.5) {
    ...
    for (let i = 0, l = points.length; i < l; i++) {
      ...
      if (split_left || split_right) {
        ...
        let np1 = p1
          .clone()
          .move({ x: n1.dy * delta, y: -n1.dx * delta, z: 0 });
        let np2 = p1
          .clone()
          .move({ x: -n2.dy * delta, y: n2.dx * delta, z: 0 });
