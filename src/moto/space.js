...
gapp.register('moto.space', [], (root, exports) => {
    ...
    if (xon && axesOn) {
      ...
      if (platformBelt) {
        for (let i = 0; i <= ruler.x2; i += grid.unitMajor) {
          context.fillText(
            (i * factor).round(1).toString(),
            ruler.x2 - i + xPadding / 2,
            0
          );
        }
      } else {
  ...
  function updateGrid() {
    let { view, unitMinor, unitMajor, colorMajor, colorMinor, colorX, colorY } =
      grid;
    let oldView = view;
    ...
    for (let x = -unitMinor; x > x1; x -= unitMinor) {
      ...
      arr.append({ x: x - xo, y: -oh, z: zp }).append({
        x: x - xo,
        y: oh,
        z: zp,
      });
    ...
    for (let y = -unitMinor; y > y1; y -= unitMinor) {
      ...
      arr.append({ x: -ow, y: y - yo, z: zp }).append({
        x: ow,
        y: y - yo,
        z: zp,
      });
  ...
  function setOrigin(x, y, z, show) {
    if (grid.origin) {
      let or = origin;
      let unchanged =
        x === or.x && y === or.y && z === or.z && show === or.show;
      if (!unchanged) {
    ...
    pi4.position.y = -0.25;
    let aa, bb, cc;
    group.add((aa = new THREE.Mesh(new THREE.RingGeometry(5, 5.5, 50), rmat)));
    group.add((bb = new THREE.Mesh(new THREE.PlaneGeometry(0.5, 10), rmat)));
    group.add((cc = new THREE.Mesh(new THREE.PlaneGeometry(10, 0.5), rmat)));
  ...
  function setRound(bool) {
    ...
    if (bool) {
      platform = new THREE.Mesh(
        new THREE.CylinderGeometry(0.5, 0.5, 1, 60),
        platformMaterial
      );
  ...
  function onMouseUp(event) {
    ...
    if (
      mouseStart &&
      mouseEnd.x - mouseStart.x + mouseEnd.y - mouseStart.y === 0
    ) {
      event.preventDefault();
      let refresh = false,
        selection = null;
  ...
  let Space = exports({
    ...
    platform: {
      ...
      showGrid: (b) => {
        showGrid = grid.view.visible = b;
      },
      showGrid2: (b) => {
        showGridLines = grid.lines.visible = b;
      },
      setMaxZ: (z) => {
        panY = z / 2;
      },
      ...
      isGridVisible() {
        return grid.view.visible;
      },
      opacity: (o) => {
        platform.material.opacity = o;
        Space.platform.setVisible(o > 0);
      },
      onMove: (f, t) => {
        (platformOnMove = f), (platformOnMoveTime = t || platformOnMoveTime);
      },
      onHover: (f) => {
        platformHover = f;
      },
      ...
      get world() {
        throw 'platform.world deprecated';
      },
    },
    ...
    view: {
      top: (then) => {
        tweenCam({ left: home, up: 0, panX, panY, panZ, then });
      },
      back: (then) => {
        tweenCam({ left: PI, up: PI2, panX, panY, panZ, then });
      },
      home: (then) => {
        tweenCam({ left: home, up, panX, panY, panZ, then });
      },
      front: (then) => {
        tweenCam({ left: 0, up: PI2, panX, panY, panZ, then });
      },
      right: (then) => {
        tweenCam({ left: PI2, up: PI2, panX, panY, panZ, then });
      },
      left: (then) => {
        tweenCam({ left: -PI2, up: PI2, panX, panY, panZ, then });
      },
    ...
    screenshot3(param = {}) {
      ...
      if (param.out) {
        ncv.toBlob((blob) =>
          blob.arrayBuffer().then((png) => param.out({ png, width, height }))
        );
      }
      return ncv;
    },
    ...
    init: (domelement, slider, ortho) => {
      ...
      viewControl = new moto.Orbit(
        camera,
        domelement,
        (position, moved) => {
          if (platform) {
            platform.visible = hidePlatformBelow
              ? initialized && position.y >= 0 && showPlatform
              : showPlatform;
      ...
      let skygrid = new THREE.Mesh(
        ...
        (skyGridMaterial = new THREE.MeshBasicMaterial({
          color: skyGridColor,
          side: THREE.DoubleSide,
        }))
      );
