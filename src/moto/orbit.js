    ...
    function touchstart(event) {
      ...
      switch (event.touches.length) {
        ...
        case 2: // two-fingered touch: dolly
          ...
          firstTouch = [...event.touches].map((v) => {
            return { x: v.pageX, y: v.pageY }
          })
          touchSlide = false
          lastDY = 0
        ...
        case 4: // four-fingered touch: pan
          ...
          const px =
            touches.map((t) => t.pageX).reduce((a, b) => a + b) / touches.length
          const py =
            touches.map((t) => t.pageY).reduce((a, b) => a + b) / touches.length
    ...
    function touchmove(event) {
      ...
      switch (event.touches.length) {
        ...
        case 2: // two-fingered touch: dolly
          ...
          let touches = [...event.touches].map((v) => {
            return { x: v.pageX, y: v.pageY }
          })
          let ddx =
            (touches[0].x -
              firstTouch[0].x +
              (touches[1].x - firstTouch[1].x)) /
            2
          let ddy =
            (touches[0].y -
              firstTouch[0].y +
              (touches[1].y - firstTouch[1].y)) /
            2
        ...
        case 4: // four-fingered touch: pan
          ...
          const px =
            tlist.map((t) => t.pageX).reduce((a, b) => a + b) / tlist.length
          const py =
            tlist.map((t) => t.pageY).reduce((a, b) => a + b) / tlist.length
