...
gapp.register('kiri.function', (root, exports) => {
  ...
  function prepareSlices(callback, scale = 1, offset = 0) {
    ...
    const totalv = slicing
      .map((w) => w.getVertices().count)
      .reduce((a, v) => a + v)
    ...
    if (isBelt && extruders.length > 1 && process.outputPurgeTower) {
      process.beltAnchor = Math.max(
        ...
        Math.sqrt(process.outputPurgeTower) *
          extruders.length *
          (1 / Math.sqrt(2))
      )
    } else if (isBelt) {
      ...
      widget.slice(
        ...
        (sliced, error) => {
          ...
          if (sliced) {
            // update segment time
            if (lastMsg) {
              segtimes[`${widget.id}_${segNumber++}_${lastMsg}`] =
                mark - startTime
            }
        ...
        (update, msg, alert) => {
          ...
          if (update >= 0) {
            ...
            show.progress(
              offset + (totalProgress / slicing.length) * scale,
              msg
            )
          }
        }
    ...
    function sliceDone() {
      ...
      for (let widget of slicing) {
        // on done
        segtimes[`${widget.id}_${segNumber++}_draw`] = widget.render(
          widget.stack
        )
        ...
        if (scale === 1) {
          ...
          widget.setOpacity(
            camOrLaser ? COLOR.cam_sliced_opacity : COLOR.sliced_opacity
          )
  ...
  function preparePreview(callback, scale = 1, offset = 0) {
    ...
    if (isCam) {
      api.widgets.opacity(
        isDark ? COLOR.cam_preview_opacity_dark : COLOR.cam_preview_opacity
      )
