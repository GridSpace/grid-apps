...
gapp.register('kiri.init', (root, exports) => {
  ...
  function onBooleanClick(el) {
    ...
    settings.device.originCenter =
      ui.deviceOrigin.checked || ui.deviceRound.checked;
  ...
  function inputHasFocus() {
    ...
    return (
      active && (active.nodeName === 'INPUT' || active.nodeName === 'TEXTAREA')
    );
  }
  ...
  function keyHandler(evt) {
    ...
    if (evt.ctrlKey) {
      switch (evt.key) {
        case 'g':
          return api.group.merge();
        case 'u':
          return api.group.split();
      }
    }
    switch (evt.charCode) {
      case cca('`'):
        ...
        break;
      case cca('0'):
        api.show.slices(api.var.layer_max);
        break;
      case cca('1'):
      ...
      case cca('3'):
        ...
        break;
      case cca('4'):
        api.show.slices((api.var.layer_max * 4) / 10);
        break;
      case cca('5'):
        api.show.slices((api.var.layer_max * 5) / 10);
        break;
      case cca('6'):
        api.show.slices((api.var.layer_max * 6) / 10);
        break;
      case cca('7'):
        api.show.slices((api.var.layer_max * 7) / 10);
        break;
      case cca('8'):
        api.show.slices((api.var.layer_max * 8) / 10);
        break;
      case cca('9'):
        api.show.slices((api.var.layer_max * 9) / 10);
        break;
      case cca('?'):
        api.help.show();
        break;
  ...
  function keys(o) {
    let key,
      list = [];
    for (key in o) {
      if (o.hasOwnProperty(key)) list.push(key);
    }
    return list.sort();
  }
  ...
  function workspaceNew() {
    uc.confirm('Clear Workspace?', { ok: true, cancel: false }).then(
      (value) => {
        if (value === true) {
  ...
  function profileExport() {
    ...
    uc.confirm('Filename', { ok: true, cancel: false }, suggestion, opt).then(
      (name) => {
        if (!name) return;
  ...
  function updateCatalog(files) {
    ...
    for (let name in files) {
      list.push({
        n: name,
        ln: name.toLowerCase(),
        v: files[name].vertices,
        t: files[name].updated,
      });
    }
  ...
  function init_one() {
    ...
    space.useDefaultKeys(
      api.feature.on_key === undefined || api.feature.on_key_defaults
    );
    updateDrawer();
    ...
    Object.assign(ui, {
      ...
      _____: {
        ...
        row: newRow([
          (anim.replay = newButton(null, 'anim.replay', {
            icon: '<i class="fas fa-fast-backward"></i>',
            title: 'restart',
          })),
          (anim.play = newButton(null, 'anim.play', {
            icon: '<i class="fas fa-play"></i>',
            title: 'play',
          })),
          (anim.pause = newButton(null, 'anim.pause', {
            icon: '<i class="fas fa-pause"></i>',
            title: 'pause',
          })),
          (anim.step = newButton(null, 'anim.step', {
            icon: '<i class="fas fa-step-forward"></i>',
            title: 'single step',
          })),
          (anim.speed = newButton(null, 'anim.fast', {
          ...
          (anim.trans = newButton(null, 'anim.trans', {
            icon: '<i class="fa-solid fa-border-none"></i>',
            title: 'transparency',
            class: 'padleft',
          })),
          (anim.model = newButton(null, 'anim.model', {
            icon: '<i class="fa-solid fa-eye"></i>',
            title: 'show model',
          })),
          (anim.shade = newButton(null, 'anim.stock', {
      ...
      _____: newDiv({
        class: 'f-col t-body t-inset',
        addto: $('dev-config'),
        set: true,
      }),
      device: newGroup(LANG.dv_gr_dev, null, {
        group: 'ddev',
        inline,
        class: 'noshow',
      }),
      ...
      bedWidth: newInput('X (width)', {
        title: LANG.dv_bedw_l,
        convert: toFloat,
        size: 6,
        units,
        round: 2,
        action: updateDeviceSize,
      }),
      bedDepth: newInput('Y (depth)', {
        title: LANG.dv_bedw_l,
        convert: toFloat,
        size: 6,
        units,
      ...
      resolutionX: newInput(LANG.dv_rezx_s, {
        title: LANG.dv_rezx_l,
        convert: toInt,
        size: 6,
        modes: SLA,
      }),
      resolutionY: newInput(LANG.dv_rezy_s, {
        title: LANG.dv_rezy_l,
        convert: toInt,
        size: 6,
        modes: SLA,
      }),
      ...
      _____: newGroup('firmware', null, {
        group: 'dext',
        inline,
        modes: NO_WEDM,
      }),
      fwRetract: newBoolean(LANG.dv_retr_s, onBooleanClick, {
        title: LANG.dv_retr_l,
        modes: FDM,
      }),
      deviceOrigin: newBoolean(LANG.dv_orgc_s, onBooleanClick, {
        title: LANG.dv_orgc_l,
        modes: FDM_LZN,
        show: () => !ui.deviceRound.checked,
      }),
      deviceRound: newBoolean(LANG.dv_bedc_s, onBooleanClick, {
        title: LANG.dv_bedc_l,
        modes: FDM,
        trigger,
        show: isNotBelt,
      }),
      deviceBelt: newBoolean(LANG.dv_belt_s, onBooleanClick, {
        title: LANG.dv_belt_l,
        modes: FDM,
        trigger,
        show: () => !ui.deviceRound.checked,
      }),
      separator: newBlank({ class: 'pop-sep', modes: FDM, driven }),
      spindleMax: newInput(LANG.dv_spmx_s, {
        title: LANG.dv_spmx_l,
        convert: toInt,
        size: 5,
        modes: CAM,
        trigger,
      }),
      deviceZMax: newInput(LANG.dv_zmax_s, {
        title: LANG.dv_zmax_l,
        convert: toInt,
        size: 5,
        modes: FDM,
      }),
      gcodeTime: newInput(LANG.dv_time_s, {
        title: LANG.dv_time_l,
        convert: toFloat,
        size: 5,
        modes: FDM,
      }),
      ...
      extFilament: newInput(LANG.dv_fila_s, {
        title: LANG.dv_fila_l,
        convert: toFloat,
        modes: FDM,
      }),
      extNozzle: newInput(LANG.dv_nozl_s, {
        title: LANG.dv_nozl_l,
        convert: toFloat,
        modes: FDM,
      }),
      extOffsetX: newInput(LANG.dv_exox_s, {
        title: LANG.dv_exox_l,
        convert: toFloat,
        modes: FDM,
      }),
      extOffsetY: newInput(LANG.dv_exoy_s, {
        title: LANG.dv_exoy_l,
        convert: toFloat,
        modes: FDM,
      }),
      ...
      extActions: newRow(
        [
          (ui.extPrev = newButton(undefined, undefined, {
            icon: '<i class="fas fa-less-than"></i>',
          })),
          (ui.extAdd = newButton(undefined, undefined, {
            icon: '<i class="fas fa-plus"></i>',
          })),
          (ui.extDel = newButton(undefined, undefined, {
            icon: '<i class="fas fa-minus"></i>',
          })),
          (ui.extNext = newButton(undefined, undefined, {
            icon: '<i class="fas fa-greater-than"></i>',
          })),
        ],
      ...
      gcodeStrip: newBoolean(LANG.dv_strc_s, onBooleanClick, {
        title: LANG.dv_strc_l,
        modes: CAM,
      }),
      gcodeSpace: newBoolean(LANG.dv_tksp_s, onBooleanClick, {
        title: LANG.dv_tksp_l,
        modes: CAM_LZR,
      }),
      laserMaxPower: newInput(LANG.ou_maxp_s, {
        title: LANG.ou_maxp_l,
        modes: LASER,
        size: 7,
        text: true,
      }),
      useLaser: newBoolean(LANG.dv_lazr_s, onBooleanClick, {
        title: LANG.dv_lazr_l,
        modes: CAM,
      }),
      gcodeFExt: newInput(LANG.dv_fext_s, {
        title: LANG.dv_fext_l,
        modes: CAM_LZR,
        size: 7,
        text: true,
      }),
      gcodeEd: newGroup(LANG.dv_gr_gco, $('dg'), {
        group: 'dgcp',
        inline,
        modes: GCODE,
      }),
      gcodeMacros: newRow(
        [
          (ui.gcodePre = newGCode(LANG.dv_head_s, {
            title: LANG.dv_head_l,
            modes: GCODE,
            area: gcode,
          })).button,
          (ui.gcodePost = newGCode(LANG.dv_foot_s, {
            title: LANG.dv_foot_l,
            modes: GCODE,
            area: gcode,
          })).button,
          (ui.gcodeLayer = newGCode(LANG.dv_layr_s, {
            title: LANG.dv_layr_l,
            modes: FDM,
            area: gcode,
          })).button,
          (ui.gcodeTrack = newGCode(LANG.dv_prog_s, {
            title: LANG.dv_prog_l,
            modes: FDM,
            area: gcode,
          })).button,
          (ui.gcodeFan = newGCode(LANG.dv_fanp_s, {
            title: LANG.dv_fanp_l,
            modes: FDM,
            area: gcode,
          })).button,
          (ui.gcodeFeature = newGCode(LANG.dv_feat_s, {
            title: LANG.dv_feat_l,
            modes: FDM,
            area: gcode,
          })).button,
          (ui.gcodeLaserOn = newGCode(LANG.dv_lzon_s, {
            title: LANG.dv_lzon_l,
            modes: LASER,
            area: gcode,
          })).button,
          (ui.gcodeLaserOff = newGCode(LANG.dv_lzof_s, {
            title: LANG.dv_lzof_l,
            modes: LASER,
            area: gcode,
          })).button,
          (ui.gcodeWaterOn = newGCode(LANG.dv_waon_s, {
            title: LANG.dv_waon_l,
            modes: WJET,
            area: gcode,
          })).button,
          (ui.gcodeWaterOff = newGCode(LANG.dv_waof_s, {
            title: LANG.dv_waof_l,
            modes: WJET,
            area: gcode,
          })).button,
          (ui.gcodeKnifeDn = newGCode(LANG.dv_dkon_s, {
            title: LANG.dv_dkon_l,
            modes: DRAG,
            area: gcode,
          })).button,
          (ui.gcodeKnifeUp = newGCode(LANG.dv_dkof_s, {
            title: LANG.dv_dkof_l,
            modes: DRAG,
            area: gcode,
          })).button,
          (ui.gcodeChange = newGCode(LANG.dv_tool_s, {
            title: LANG.dv_tool_l,
            modes: FDM_CAM,
            area: gcode,
          })).button,
          (ui.gcodeDwell = newGCode(LANG.dv_dwll_s, {
            title: LANG.dv_dwll_l,
            modes: CAM,
            area: gcode,
          })).button,
          (ui.gcodeSpindle = newGCode(LANG.dv_sspd_s, {
            title: LANG.dv_sspd_l,
            modes: CAM,
            area: gcode,
            show: () => ui.spindleMax.value > 0,
          })).button,
        ],
      ...
      antiAlias: newBoolean(LANG.op_anta_s, booleanSave, {
        title: LANG.op_anta_l,
      }),
      reverseZoom: newBoolean(LANG.op_invr_s, booleanSave, {
        title: LANG.op_invr_l,
      }),
      ortho: newBoolean(LANG.op_orth_s, booleanSave, {
        title: LANG.op_orth_l,
      }),
      dark: newBoolean(LANG.op_dark_s, booleanSave, {
        title: LANG.op_dark_l,
      }),
      drawer: newBoolean('slide out', booleanSave, {
        title: 'slide out settings drawer',
      }),
      scrolls: newBoolean('scrollbars', booleanSave, {
        title: 'show scrollbars',
      }),
      devel: newBoolean(LANG.op_devl_s, booleanSave, {
        title: LANG.op_devl_l,
      }),
      _____: newGroup(LANG.op_disp, $('prefs-gen2'), { inline }),
      showOrigin: newBoolean(LANG.op_shor_s, booleanSave, {
        title: LANG.op_shor_l,
      }),
      showRulers: newBoolean(LANG.op_shru_s, booleanSave, {
        title: LANG.op_shru_l,
      }),
      showSpeeds: newBoolean(LANG.op_sped_s, speedSave, {
        title: LANG.op_sped_l,
      }),
      shiny: newBoolean(LANG.op_shny_s, booleanSave, {
        title: LANG.op_shny_l,
        modes: FDM,
      }),
      lineType: newSelect(
        LANG.op_line_s,
        { title: LANG.op_line_l, action: lineTypeSave, modes: FDM },
        'linetype'
      ),
      animesh: newSelect(
        LANG.op_anim_s,
        { title: LANG.op_anim_l, action: aniMeshSave, modes: CAM },
        'animesh'
      ),
      units: newSelect(
        LANG.op_unit_s,
        { title: LANG.op_unit_l, action: unitsSave, modes: CAM },
        'units'
      ),
      edgeangle: newInput(LANG.op_spoa_s, {
        title: LANG.op_spoa_l,
        convert: toFloat,
        size: 3,
      }),
      _____: newGroup(LANG.lo_menu, $('prefs-lay'), { inline }),
      autoSave: newBoolean(LANG.op_save_s, booleanSave, {
        title: LANG.op_save_l,
      }),
      autoLayout: newBoolean(LANG.op_auto_s, booleanSave, {
        title: LANG.op_auto_l,
      }),
      freeLayout: newBoolean(LANG.op_free_s, booleanSave, {
        title: LANG.op_free_l,
      }),
      spaceRandoX: newBoolean(LANG.op_spcx_s, booleanSave, {
        title: LANG.op_spcx_l,
        show: isBelt,
      }),
      spaceLayout: newInput(LANG.op_spcr_s, {
        title: LANG.op_spcr_l,
        convert: toFloat,
        size: 3,
        units,
      }),
      _____: newGroup(LANG.xp_menu, $('prefs-xpo'), { inline: true }),
      exportLocal: newBoolean(`Grid:Local`, booleanSave, {
        title: LANG.op_exgl_l,
      }),
      exportGhost: newBoolean(`Grid:Host`, booleanSave, {
        title: LANG.op_exgh_l,
      }),
      exportOcto: newBoolean(`OctoPrint`, booleanSave, {
        title: LANG.op_exop_l,
      }),
      ...
      detail: newSelect(
        LANG.pt_qual_s,
        { title: LANG.pt_qual_l, action: detailSave },
        'detail'
      ),
      healMesh: newBoolean(LANG.pt_heal_s, booleanSave, {
        title: LANG.pt_heal_l,
      }),
      threaded: newBoolean(LANG.pt_thrd_s, booleanSave, {
        title: LANG.pt_thrd_l,
        modes: THREED,
      }),
      assembly: newBoolean(LANG.pt_assy_s, booleanSave, {
        title: LANG.pt_assy_l,
        modes: THREED,
      }),
      ...
      _____: newGroup(LANG.sl_menu, $('fdm-layers'), {
        modes: FDM,
        driven,
        hideable,
        separator,
      ...
      sliceHeight: newInput(LANG.sl_lahi_s, {
        title: LANG.sl_lahi_l,
        convert: toFloat,
      }),
      sliceMinHeight: newInput(LANG.ad_minl_s, {
        title: LANG.ad_minl_l,
        convert: toFloat,
        bound: bound(0, 3.0),
        show: () => ui.sliceAdaptive.checked,
      }),
      sliceTopLayers: newInput(LANG.sl_ltop_s, {
        title: LANG.sl_ltop_l,
        convert: toInt,
      }),
      sliceBottomLayers: newInput(LANG.sl_lbot_s, {
        title: LANG.sl_lbot_l,
        convert: toInt,
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      sliceAdaptive: newBoolean(LANG.ad_adap_s, onBooleanClick, {
        title: LANG.ad_adap_l,
      }),
      _____: newGroup(LANG.sw_menu, $('fdm-walls'), {
        modes: FDM,
        driven,
        hideable,
        separator,
      ...
      sliceShells: newInput(LANG.sl_shel_s, {
        title: LANG.sl_shel_l,
        convert: toFloat,
      }),
      sliceLineWidth: newInput(LANG.sl_line_s, {
        title: LANG.sl_line_l,
        convert: toFloat,
        bound: bound(0, 5),
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      sliceShellOrder: newSelect(
        LANG.sl_ordr_s,
        { title: LANG.sl_ordr_l },
        'shell'
      ),
      sliceDetectThin: newSelect(
        LANG.ad_thin_s,
        { title: LANG.ad_thin_l, action: thinWallSave },
        'thin'
      ),
      outputAlternating: newBoolean(LANG.ad_altr_s, onBooleanClick, {
        title: LANG.ad_altr_l,
      }),
      sliceZInterleave: newBoolean(LANG.ad_zint_s, onBooleanClick, {
        title: LANG.ad_zint_l,
        show: zIntShow,
      }),
      _____: newGroup(LANG.fi_menu, $('fdm-fill'), {
        modes: FDM,
        driven,
        hideable,
        separator,
      ...
      sliceFillSparse: newInput(LANG.fi_pcnt_s, {
        title: LANG.fi_pcnt_l,
        convert: toFloat,
        bound: bound(0.0, 1.0),
        show: hasInfill,
      }),
      sliceFillRepeat: newInput(LANG.fi_rept_s, {
        title: LANG.fi_rept_l,
        convert: toInt,
        bound: bound(1, 10),
        show: fillIsLinear,
      }),
      sliceFillOverlap: newInput(LANG.fi_over_s, {
        title: LANG.fi_over_l,
        convert: toFloat,
        bound: bound(0.0, 2.0),
        show: hasInfill,
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      sliceFillRate: newInput(LANG.ou_feed_s, {
        title: LANG.ou_feed_l,
        convert: toInt,
        bound: bound(0, 500),
      }),
      sliceSolidRate: newInput(LANG.ou_fini_s, {
        title: LANG.ou_fini_l,
        convert: toInt,
        bound: bound(0, 500),
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      sliceFillGrow: newInput(LANG.fi_grow_s, {
        title: LANG.fi_grow_l,
        convert: toFloat,
      }),
      sliceFillAngle: newInput(LANG.fi_angl_s, {
        title: LANG.fi_angl_l,
        convert: toFloat,
      }),
      _____: newGroup(LANG.fh_menu, $('fdm-heat'), {
        modes: FDM,
        driven,
        hideable,
        separator,
      ...
      outputTemp: newInput(LANG.ou_nozl_s, {
        title: LANG.ou_nozl_l,
        convert: toInt,
      }),
      outputBedTemp: newInput(LANG.ou_bedd_s, {
        title: LANG.ou_bedd_l,
        convert: toInt,
      }),
      _____: newGroup(LANG.fc_menu, $('fdm-cool'), {
        modes: FDM,
        driven,
        hideable,
        separator,
      ...
      outputFanLayer: newInput(LANG.ou_fanl_s, {
        title: LANG.ou_fanl_l,
        convert: toInt,
        bound: bound(0, 255),
      }),
      outputFanSpeed: newInput(LANG.ou_fans_s, {
        title: LANG.ou_fans_l,
        convert: toInt,
        bound: bound(0, 255),
      }),
      _____: newGroup(LANG.sp_menu, $('fdm-support'), {
        modes: FDM,
        driven,
        hideable,
        separator,
      ...
      sliceSupportNozzle: newSelect(
        LANG.sp_nozl_s,
        { title: LANG.sp_nozl_l, show: isMultiHead },
        'extruders'
      ),
      sliceSupportDensity: newInput(LANG.sp_dens_s, {
        title: LANG.sp_dens_l,
        convert: toFloat,
        bound: bound(0.0, 1.0),
      }),
      sliceSupportSize: newInput(LANG.sp_size_s, {
        title: LANG.sp_size_l,
        convert: toFloat,
        bound: bound(1.0, 200.0),
      }),
      sliceSupportOffset: newInput(LANG.sp_offs_s, {
        title: LANG.sp_offs_l,
        convert: toFloat,
        bound: bound(0.0, 200.0),
      }),
      sliceSupportGap: newInput(LANG.sp_gaps_s, {
        title: LANG.sp_gaps_l,
        convert: toInt,
        bound: bound(0, 5),
      }),
      sliceSupportArea: newInput(LANG.sp_area_s, {
        title: LANG.sp_area_l,
        convert: toFloat,
        bound: bound(0.0, 200.0),
      }),
      sliceSupportExtra: newInput(LANG.sp_xpnd_s, {
        title: LANG.sp_xpnd_l,
        convert: toFloat,
        bound: bound(0.0, 10.0),
      }),
      sliceSupportGrow: newInput(LANG.sp_grow_s, {
        title: LANG.sp_grow_l,
        convert: toFloat,
        bound: bound(0.0, 10.0),
      }),
      sliceSupportAngle: newInput(LANG.sp_angl_s, {
        title: LANG.sp_angl_l,
        convert: toFloat,
        bound: bound(0.0, 90.0),
      }),
      sliceSupportSpan: newInput(LANG.sp_span_s, {
        title: LANG.sp_span_l,
        convert: toFloat,
        bound: bound(0.0, 200.0),
        show: () => ui.sliceSupportEnable.checked,
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      sliceSupportEnable: newBoolean(LANG.sp_auto_s, onBooleanClick, {
        title: LANG.sp_auto_l,
        show: isNotBelt,
      }),
      sliceSupportOutline: newBoolean(LANG.sp_outl_s, onBooleanClick, {
        title: LANG.sp_outl_l,
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      sliceSupportGen: newRow(
        [
          (ui.ssaGen = newButton(LANG.sp_detect, onButtonClick, {
            class: 'f-col grow a-center',
          })),
        ],
      ...
      sliceSupportManual: newRow(
        [
          (ui.ssmAdd = newButton(undefined, onButtonClick, {
            icon: '<i class="fas fa-plus"></i>',
          })),
          (ui.ssmDun = newButton(undefined, onButtonClick, {
            icon: '<i class="fas fa-check"></i>',
          })),
          (ui.ssmClr = newButton(undefined, onButtonClick, {
            icon: '<i class="fas fa-trash-alt"></i>',
          })),
        ],
        { class: 'ext-buttons f-row' }
      ),
      _____: newGroup(LANG.fl_menu, $('fdm-base'), {
        modes: FDM,
        driven,
        hideable,
        separator,
      ...
      firstSliceHeight: newInput(LANG.fl_lahi_s, {
        title: LANG.fl_lahi_l,
        convert: toFloat,
        show: isNotBelt,
      }),
      firstLayerNozzleTemp: newInput(LANG.fl_nozl_s, {
        title: LANG.fl_nozl_l,
        convert: toInt,
        show: isNotBelt,
      }),
      firstLayerBedTemp: newInput(LANG.fl_bedd_s, {
        title: LANG.fl_bedd_l,
        convert: toInt,
        show: isNotBelt,
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      firstLayerFanSpeed: newInput(LANG.ou_fans_s, {
        title: LANG.ou_fans_l,
        convert: toInt,
        bound: bound(0, 255),
        show: isBelt,
      }),
      firstLayerYOffset: newInput(LANG.fl_zoff_s, {
        title: LANG.fl_zoff_l,
        convert: toFloat,
        show: isBelt,
      }),
      firstLayerFlatten: newInput(LANG.fl_flat_s, {
        title: LANG.fl_flat_l,
        convert: toFloat,
        show: isBelt,
      }),
      firstLayerRate: newInput(LANG.fl_rate_s, {
        title: LANG.fl_rate_l,
        convert: toFloat,
      }),
      firstLayerFillRate: newInput(LANG.fl_frat_s, {
        title: LANG.fl_frat_l,
        convert: toFloat,
        show: isNotBelt,
      }),
      separator: newBlank({ class: 'set-sep', driven, show: isNotBelt }),
      firstLayerLineMult: newInput(LANG.fl_sfac_s, {
        title: LANG.fl_sfac_l,
        convert: toFloat,
        bound: bound(0.5, 2),
        show: isNotBelt,
      }),
      firstLayerPrintMult: newInput(LANG.fl_mult_s, {
        title: LANG.fl_mult_l,
        convert: toFloat,
      }),
      separator: newBlank({ class: 'set-sep', driven, show: isBelt }),
      firstLayerBrim: newInput(LANG.fl_brim_s, {
        title: LANG.fl_brim_l,
        convert: toInt,
        show: isBelt,
      }),
      firstLayerBrimIn: newInput(LANG.fl_brin_s, {
        title: LANG.fl_brin_l,
        convert: toInt,
        show: isBelt,
      }),
      firstLayerBrimTrig: newInput(LANG.fl_brmn_s, {
        title: LANG.fl_brmn_l,
        convert: toInt,
        show: isBelt,
      }),
      firstLayerBrimGap: newInput(LANG.fl_brgp_s, {
        title: LANG.fl_brgp_l,
        convert: toFloat,
        show: isBelt,
      }),
      separator: newBlank({ class: 'set-sep', driven, show: isBelt }),
      firstLayerBeltLead: newInput(LANG.fl_bled_s, {
        title: LANG.fl_bled_l,
        convert: toFloat,
        show: isBelt,
      }),
      firstLayerBeltBump: newInput(LANG.fl_blmp_s, {
        title: LANG.fl_blmp_l,
        convert: toFloat,
        bound: bound(0, 10),
        show: isBelt,
      }),
      separator: newBlank({ class: 'set-sep', driven, show: isNotBelt }),
      outputBrimCount: newInput(LANG.fl_skrt_s, {
        title: LANG.fl_skrt_l,
        convert: toInt,
        show: isNotBelt,
      }),
      outputBrimOffset: newInput(LANG.fl_skro_s, {
        title: LANG.fl_skro_l,
        convert: toFloat,
        show: isNotBelt,
      }),
      outputRaftSpacing: newInput(LANG.fr_spac_s, {
        title: LANG.fr_spac_l,
        convert: toFloat,
        bound: bound(0.0, 3.0),
        show: () => ui.outputRaft.checked && isNotBelt(),
      }),
      separator: newBlank({ class: 'set-sep', driven, show: isNotBelt }),
      outputRaft: newBoolean(LANG.fr_nabl_s, onBooleanClick, {
        title: LANG.fr_nabl_l,
        trigger,
        show: () => isNotBelt(),
      }),
      outputDraftShield: newBoolean(LANG.fr_draf_s, onBooleanClick, {
        title: LANG.fr_draf_l,
        trigger,
        show: () => isNotBelt(),
      }),
      _____: newGroup(LANG.ou_menu, $('fdm-output'), {
        modes: FDM,
        driven,
        hideable,
        separator,
      ...
      outputFeedrate: newInput(LANG.ou_feed_s, {
        title: LANG.ou_feed_l,
        convert: toInt,
      }),
      outputFinishrate: newInput(LANG.ou_fini_s, {
        title: LANG.ou_fini_l,
        convert: toInt,
      }),
      outputSeekrate: newInput(LANG.ou_move_s, {
        title: LANG.ou_move_l,
        convert: toInt,
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      outputShellMult: newInput(LANG.ou_shml_s, {
        title: LANG.ou_exml_l,
        convert: toFloat,
        bound: bound(0.0, 2.0),
      }),
      outputFillMult: newInput(LANG.ou_flml_s, {
        title: LANG.ou_exml_l,
        convert: toFloat,
        bound: bound(0.0, 2.0),
      }),
      outputSparseMult: newInput(LANG.ou_spml_s, {
        title: LANG.ou_exml_l,
        convert: toFloat,
        bound: bound(0.0, 2.0),
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      outputRetractDist: newInput(LANG.ad_rdst_s, {
        title: LANG.ad_rdst_l,
        convert: toFloat,
      }),
      outputRetractSpeed: newInput(LANG.ad_rrat_s, {
        title: LANG.ad_rrat_l,
        convert: toInt,
      }),
      outputRetractWipe: newInput(LANG.ad_wpln_s, {
        title: LANG.ad_wpln_l,
        convert: toFloat,
        bound: bound(0.0, 10),
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      sliceLayerStart: newSelect(
        LANG.sl_strt_s,
        { title: LANG.sl_strt_l },
        'start'
      ),
      outputLayerRetract: newBoolean(LANG.ad_lret_s, onBooleanClick, {
        title: LANG.ad_lret_l,
      }),
      outputAvoidGaps: newBoolean(LANG.ad_agap_s, onBooleanClick, {
        title: LANG.ad_agap_l,
      }),
      separator: newBlank({ class: 'set-sep', driven, show: isBelt }),
      outputBeltFirst: newBoolean(LANG.ad_lbir_s, onBooleanClick, {
        title: LANG.ad_lbir_l,
        show: isBelt,
      }),
      _____: newGroup(LANG.ad_menu, $('fdm-expert'), {
        modes: FDM,
        driven,
        hideable,
        separator,
      ...
      sliceAngle: newInput(LANG.sl_angl_s, {
        title: LANG.sl_angl_l,
        convert: toFloat,
        show: isBelt,
      }),
      outputRetractDwell: newInput(LANG.ad_rdwl_s, {
        title: LANG.ad_rdwl_l,
        convert: toInt,
      }),
      sliceSolidMinArea: newInput(LANG.ad_msol_s, {
        title: LANG.ad_msol_l,
        convert: toFloat,
      }),
      outputMinSpeed: newInput(LANG.ad_mins_s, {
        title: LANG.ad_mins_l,
        convert: toFloat,
        bound: bound(1, 200),
      }),
      outputShortPoly: newInput(LANG.ad_spol_s, {
        title: LANG.ad_spol_l,
        convert: toFloat,
        bound: bound(0, 10000),
      }),
      outputCoastDist: newInput(LANG.ad_scst_s, {
        title: LANG.ad_scst_l,
        convert: toFloat,
        bound: bound(0.0, 10),
      }),
      zHopDistance: newInput(LANG.ad_zhop_s, {
        title: LANG.ad_zhop_l,
        convert: toFloat,
        bound: bound(0, 3.0),
      }),
      arcTolerance: newInput(LANG.ad_arct_s, {
        title: LANG.ad_arct_l,
        convert: toFloat,
        bound: bound(0, 1.0),
        show: () => {
          return isNotBelt();
      ...
      antiBacklash: newInput(LANG.ad_abkl_s, {
        title: LANG.ad_abkl_l,
        convert: toInt,
        bound: bound(0, 3),
      }),
      outputLoops: newInput(LANG.ag_loop_s, {
        title: LANG.ag_loop_l,
        convert: toInt,
        bound: bound(-1, 1000),
        show: isBelt,
      }),
      outputPurgeTower: newInput(LANG.ad_purg_s, {
        title: LANG.ad_purg_l,
        convert: toInt,
        bound: bound(0, 1000),
      }),
      ...
      _____: newGroup(LANG.ct_menu, $('cam-tabs'), {
        modes: CAM,
        marker: true,
        driven,
        separator,
      }),
      camTabsWidth: newInput(LANG.ct_wdth_s, {
        title: LANG.ct_wdth_l,
        convert: toFloat,
        bound: bound(0.005, 100),
        units,
      }),
      camTabsHeight: newInput(LANG.ct_hght_s, {
        title: LANG.ct_hght_l,
        convert: toFloat,
        bound: bound(0.005, 100),
        units,
      }),
      camTabsDepth: newInput(LANG.ct_dpth_s, {
        title: LANG.ct_dpth_l,
        convert: toFloat,
        bound: bound(0.005, 100),
        units,
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      camTabsMidline: newBoolean(LANG.ct_midl_s, onBooleanClick, {
        title: LANG.ct_midl_l,
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      camTabsManual: newRow(
        [
          (ui.tabAdd = newButton(undefined, onButtonClick, {
            icon: '<i class="fas fa-plus"></i>',
          })),
          (ui.tabDun = newButton(undefined, onButtonClick, {
            icon: '<i class="fas fa-check"></i>',
          })),
          (ui.tabClr = newButton(undefined, onButtonClick, {
            icon: '<i class="fas fa-trash-alt"></i>',
          })),
        ],
        { class: 'ext-buttons f-row' }
      ),
      _____: newGroup(LANG.cs_menu, $('cam-stock'), {
        modes: CAM,
        driven,
        separator,
      }),
      camStockX: newInput(LANG.cs_wdth_s, {
        title: LANG.cs_wdth_l,
        convert: toFloat,
        bound: bound(0, 9999),
        units,
      }),
      camStockY: newInput(LANG.cs_dpth_s, {
        title: LANG.cs_dpth_l,
        convert: toFloat,
        bound: bound(0, 9999),
        units,
      }),
      camStockZ: newInput(LANG.cs_hght_s, {
        title: LANG.cs_hght_l,
        convert: toFloat,
        bound: bound(0, 9999),
        units,
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      camStockOffset: newBoolean(LANG.cs_offs_s, onBooleanClick, {
        title: LANG.cs_offs_l,
      }),
      camStockClipTo: newBoolean(LANG.cs_clip_s, onBooleanClick, {
        title: LANG.cs_clip_l,
      }),
      camStockIndexed: newBoolean(LANG.cs_indx_s, onBooleanClick, {
        title: LANG.cs_indx_l,
      }),
      camStockIndexGrid: newBoolean(LANG.cs_ishg_s, onBooleanClick, {
        title: LANG.cs_ishg_l,
        show: () => ui.camStockIndexed.checked,
      }),
      _____: newGroup(LANG.cc_menu, $('cam-limits'), {
        modes: CAM,
        driven,
        separator,
      }),
      camZAnchor: newSelect(
        LANG.ou_zanc_s,
        {
          title: LANG.ou_zanc_l,
          action: zAnchorSave,
          show: () => !ui.camStockIndexed.checked,
        },
        'zanchor'
      ),
      camZOffset: newInput(LANG.ou_ztof_s, {
        title: LANG.ou_ztof_l,
        convert: toFloat,
        units,
      }),
      camZTop: newInput(LANG.ou_ztop_s, {
        title: LANG.ou_ztop_l,
        convert: toFloat,
        units,
        trigger,
      }),
      camZBottom: newInput(LANG.ou_zbot_s, {
        title: LANG.ou_zbot_l,
        convert: toFloat,
        units,
        trigger,
      }),
      camZThru: newInput(LANG.ou_ztru_s, {
        title: LANG.ou_ztru_l,
        convert: toFloat,
        bound: bound(0.0, 100),
        units,
      }),
      camZClearance: newInput(LANG.ou_zclr_s, {
        title: LANG.ou_zclr_l,
        convert: toFloat,
        bound: bound(0.01, 100),
        units,
      }),
      camFastFeedZ: newInput(LANG.cc_rzpd_s, {
        title: LANG.cc_rzpd_l,
        convert: toFloat,
        units,
      }),
      camFastFeed: newInput(LANG.cc_rapd_s, {
        title: LANG.cc_rapd_l,
        convert: toFloat,
        units,
      }),
      _____: newGroup(LANG.ou_menu, $('cam-output'), {
        modes: CAM,
        driven,
        separator,
        group: 'cam-output',
      }),
      camConventional: newBoolean(LANG.ou_conv_s, onBooleanClick, {
        title: LANG.ou_conv_l,
      }),
      camEaseDown: newBoolean(LANG.cr_ease_s, onBooleanClick, {
        title: LANG.cr_ease_l,
      }),
      camDepthFirst: newBoolean(LANG.ou_depf_s, onBooleanClick, {
        title: LANG.ou_depf_l,
      }),
      camToolInit: newBoolean(LANG.ou_toin_s, onBooleanClick, {
        title: LANG.ou_toin_l,
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      camFirstZMax: newBoolean(LANG.ou_z1st_s, onBooleanClick, {
        title: LANG.ou_z1st_l,
      }),
      camForceZMax: newBoolean(LANG.ou_forz_s, onBooleanClick, {
        title: LANG.ou_forz_l,
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      camEaseAngle: newInput(LANG.ou_eang_s, {
        title: LANG.ou_eang_l,
        convert: toFloat,
        bound: bound(0.1, 85),
        show: () => ui.camEaseDown.checked,
      }),
      camFullEngage: newInput(LANG.ou_feng_s, {
        title: LANG.ou_feng_l,
        convert: toFloat,
        bound: bound(0.1, 1.0),
      }),
      _____: newGroup(LANG.co_menu, $('cam-origin'), {
        modes: CAM,
        driven,
        separator,
      }),
      camOriginTop: newBoolean(LANG.or_topp_s, onBooleanClick, {
        title: LANG.or_topp_l,
      }),
      camOriginCenter: newBoolean(LANG.or_cntr_s, onBooleanClick, {
        title: LANG.or_cntr_l,
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      camOriginOffX: newInput(LANG.co_offx_s, {
        title: LANG.co_offx_l,
        convert: toFloat,
        units,
      }),
      camOriginOffY: newInput(LANG.co_offy_s, {
        title: LANG.co_offy_l,
        convert: toFloat,
        units,
      }),
      camOriginOffZ: newInput(LANG.co_offz_s, {
        title: LANG.co_offz_l,
        convert: toFloat,
        units,
      }),
      _____: newGroup(LANG.op_xprt_s, $('cam-expert'), {
        group: 'cam_expert',
        modes: CAM,
        marker: false,
        driven,
      ...
      camExpertFast: newBoolean(LANG.cx_fast_s, onBooleanClick, {
        title: LANG.cx_fast_l,
        show: () => !ui.camTrueShadow.checked,
      }),
      camTrueShadow: newBoolean(LANG.cx_true_s, onBooleanClick, {
        title: LANG.cx_true_l,
        show: () => !ui.camExpertFast.checked,
      }),

      /** LASER/DRAG/WJET/WEDM cut tool Settings */

      _____: newGroup(LANG.sl_menu, $('lzr-slice'), {
        modes: TWOD,
        driven,
        separator,
      }),
      ctSliceKerf: newInput(LANG.ls_offs_s, {
        title: LANG.ls_offs_l,
        convert: toFloat,
      }),
      ctSliceHeight: newInput(LANG.ls_lahi_s, {
        title: LANG.ls_lahi_l,
        convert: toFloat,
        trigger,
      }),
      ctSliceHeightMin: newInput(LANG.ls_lahm_s, {
        title: LANG.ls_lahm_l,
        convert: toFloat,
        show: () => ui.ctSliceHeight.value == 0 && !ui.ctSliceSingle.checked,
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      ctSliceSingle: newBoolean(LANG.ls_sngl_s, onBooleanClick, {
        title: LANG.ls_sngl_l,
      }),
      ctOmitInner: newBoolean(LANG.we_omit_s, onBooleanClick, {
        title: LANG.we_omit_l,
        modes: WEDM,
      }),
      _____: newGroup('surfaces', $('lzr-surface'), {
        modes: [-1],
        driven,
        separator,
      }),
      ctSurfaces: newRow(
        [
          (ui.faceAdd = newButton(undefined, onButtonClick, {
            icon: '<i class="fas fa-plus"></i>',
          })),
          (ui.faceDun = newButton(undefined, onButtonClick, {
            icon: '<i class="fas fa-check"></i>',
          })),
          (ui.faceClr = newButton(undefined, onButtonClick, {
            icon: '<i class="fas fa-trash-alt"></i>',
          })),
        ],
        { class: 'ext-buttons f-row', modes: WEDM }
      ),
      _____: newGroup(LANG.dk_menu, $('lzr-knife'), {
        modes: DRAG,
        marker: true,
        driven,
        separator,
      }),
      ctOutKnifeDepth: newInput(LANG.dk_dpth_s, {
        title: LANG.dk_dpth_l,
        convert: toFloat,
        bound: bound(0.0, 5.0),
      }),
      ctOutKnifePasses: newInput(LANG.dk_pass_s, {
        title: LANG.dk_pass_l,
        convert: toInt,
        bound: bound(0, 5),
      }),
      ctOutKnifeTip: newInput(LANG.dk_offs_s, {
        title: LANG.dk_offs_l,
        convert: toFloat,
        bound: bound(0.0, 10.0),
      }),
      _____: newGroup(LANG.lo_menu, $('lzr-layout'), {
        modes: TWOD,
        driven,
        separator,
      }),
      ctOutTileSpacing: newInput(LANG.ou_spac_s, {
        title: LANG.ou_spac_l,
        convert: toInt,
      }),
      ctOutMerged: newBoolean(LANG.ou_mrgd_s, onBooleanClick, {
        title: LANG.ou_mrgd_l,
        modes: TWONED,
        show: () => !ui.ctOutStack.checked,
      }),
      ctOutGroup: newBoolean(LANG.ou_grpd_s, onBooleanClick, {
        title: LANG.ou_grpd_l,
        show: () => !(ui.ctOutMark.checked || ui.ctOutStack.checked),
      }),
      _____: newGroup(LANG.ou_menu, $('lzr-output'), {
        modes: TWOD,
        driven,
        separator,
        group: 'lzr-output',
      }),
      ctOutPower: newInput(LANG.ou_powr_s, {
        title: LANG.ou_powr_l,
        convert: toInt,
        bound: bound(1, 100),
        modes: TWONED,
      }),
      ctOutSpeed: newInput(LANG.ou_sped_s, {
        title: LANG.ou_sped_l,
        convert: toInt,
      }),
      ctAdaptive: newBoolean('adaptive speed', onBooleanClick, {
        modes: WEDM,
        title:
          'controller determines best cutting speed based on material feedback at runtime',
      }),
      separator: newBlank({ class: 'set-sep', driven }),
      ctOriginBounds: newBoolean(LANG.or_bnds_s, onBooleanClick, {
        title: LANG.or_bnds_l,
        show: () => !ui.ctOriginCenter.checked,
      }),
      ctOriginCenter: newBoolean(LANG.or_cntr_s, onBooleanClick, {
        title: LANG.or_cntr_l,
        show: () => !ui.ctOriginBounds.checked,
      }),
      separator: newBlank({
        class: 'set-sep',
        driven,
      ...
      ctOriginOffX: newInput(LANG.or_offx_s, {
        title: LANG.or_offx_l,
        convert: toFloat,
        modes: WEDM,
        show: () => ui.ctOriginBounds.checked,
      }),
      ctOriginOffY: newInput(LANG.or_offy_s, {
        title: LANG.or_offy_l,
        convert: toFloat,
        modes: WEDM,
        show: () => ui.ctOriginBounds.checked,
      }),
      separator: newBlank({ class: 'set-sep', driven, modes: TWONED }),
      ctOutZColor: newBoolean(LANG.ou_layo_s, onBooleanClick, {
        title: LANG.ou_layo_l,
        modes: TWONED,
        show: () => !ui.ctOutMerged.checked,
      }),
      ctOutLayer: newBoolean(LANG.ou_layr_s, onBooleanClick, {
        title: LANG.ou_layr_l,
        modes: TWONED,
        show: () => !ui.ctOutStack.checked,
      }),
      ctOutMark: newBoolean(LANG.ou_lays_s, onBooleanClick, {
        title: LANG.ou_lays_l,
        modes: TWONED,
        show: () => !ui.ctOutStack.checked,
      }),
      separator: newBlank({ class: 'set-sep', driven, modes: LASER }),
      ctOutInches: newBoolean(LANG.ou_inch_s, onBooleanClick, {
        title: LANG.ou_inch_l,
        modes: LASER,
      }),
      ctOutStack: newBoolean(LANG.ou_stak_s, onBooleanClick, {
        title: LANG.ou_stak_l,
        modes: LASER,
      }),
      ctOutShaper: newBoolean(LANG.ou_shap_s, onBooleanClick, {
        title: LANG.ou_shap_l,
        modes: LASER,
        show: () => ui.ctOutStack.checked,
      }),
      ...
      slaProc: newGroup(LANG.sa_menu, $('sla-slice'), {
        modes: SLA,
        group: 'sla-slice',
        driven,
        separator,
      }),
      slaSlice: newInput(LANG.sa_lahe_s, {
        title: LANG.sa_lahe_l,
        convert: toFloat,
      }),
      slaShell: newInput(LANG.sa_shel_s, {
        title: LANG.sa_shel_l,
        convert: toFloat,
      }),
      slaOpenTop: newBoolean(LANG.sa_otop_s, onBooleanClick, {
        title: LANG.sa_otop_l,
      }),
      slaOpenBase: newBoolean(LANG.sa_obas_s, onBooleanClick, {
        title: LANG.sa_obas_l,
      }),
      slaLayers: newGroup(LANG.sa_layr_m, $('sla-layers'), {
        modes: SLA,
        group: 'sla-layers',
        driven,
        separator,
      }),
      slaLayerOn: newInput(LANG.sa_lton_s, {
        title: LANG.sa_lton_l,
        convert: toFloat,
      }),
      slaLayerOff: newInput(LANG.sa_ltof_s, {
        title: LANG.sa_ltof_l,
        convert: toFloat,
      }),
      slaPeelDist: newInput(LANG.sa_pldi_s, {
        title: LANG.sa_pldi_l,
        convert: toFloat,
      }),
      slaPeelLiftRate: newInput(LANG.sa_pllr_s, {
        title: LANG.sa_pllr_l,
        convert: toFloat,
      }),
      slaPeelDropRate: newInput(LANG.sa_pldr_s, {
        title: LANG.sa_pldr_l,
        convert: toFloat,
      }),
      slaBase: newGroup(LANG.sa_base_m, $('sla-base'), {
        modes: SLA,
        group: 'sla-base',
        driven,
        separator,
      }),
      slaBaseLayers: newInput(LANG.sa_balc_s, {
        title: LANG.sa_balc_l,
        convert: toInt,
      }),
      slaBaseOn: newInput(LANG.sa_lton_s, {
        title: LANG.sa_bltn_l,
        convert: toFloat,
      }),
      slaBaseOff: newInput(LANG.sa_ltof_s, {
        title: LANG.sa_bltf_l,
        convert: toFloat,
      }),
      slaBasePeelDist: newInput(LANG.sa_pldi_s, {
        title: LANG.sa_pldi_l,
        convert: toFloat,
      }),
      slaBasePeelLiftRate: newInput(LANG.sa_pllr_s, {
        title: LANG.sa_pllr_l,
        convert: toFloat,
      }),
      slaFill: newGroup(LANG.sa_infl_m, $('sla-fill'), {
        modes: SLA,
        group: 'sla-infill',
        driven,
        separator,
      }),
      slaFillDensity: newInput(LANG.sa_ifdn_s, {
        title: LANG.sa_ifdn_l,
        convert: toFloat,
        bound: bound(0, 1),
      }),
      slaFillLine: newInput(LANG.sa_iflw_s, {
        title: LANG.sa_iflw_l,
        convert: toFloat,
        bound: bound(0, 5),
      }),
      slaSupport: newGroup(LANG.sa_supp_m, $('sla-support'), {
        modes: SLA,
        group: 'sla-support',
        driven,
        separator,
      }),
      slaSupportLayers: newInput(LANG.sa_slyr_s, {
        title: LANG.sa_slyr_l,
        convert: toInt,
        bound: bound(5, 100),
      }),
      slaSupportGap: newInput(LANG.sa_slgp_s, {
        title: LANG.sa_slgp_l,
        convert: toInt,
        bound: bound(3, 30),
      }),
      slaSupportDensity: newInput(LANG.sa_sldn_s, {
        title: LANG.sa_sldn_l,
        convert: toFloat,
        bound: bound(0.01, 0.9),
      }),
      slaSupportSize: newInput(LANG.sa_slsz_s, {
        title: LANG.sa_slsz_l,
        convert: toFloat,
        bound: bound(0.1, 1),
      }),
      slaSupportPoints: newInput(LANG.sa_slpt_s, {
        title: LANG.sa_slpt_l,
        convert: toInt,
        bound: bound(3, 10),
      }),
      slaSupportEnable: newBoolean(LANG.enable, onBooleanClick, {
        title: LANG.sl_slen_l,
      }),
      slaOutput: newGroup(LANG.sa_outp_m, $('sla-output'), {
        modes: SLA,
        driven,
        separator,
        group: 'sla-output',
      }),
      slaFirstOffset: newInput(LANG.sa_opzo_s, {
        title: LANG.sa_opzo_l,
        convert: toFloat,
        bound: bound(0, 1),
      }),
      slaAntiAlias: newSelect(
        LANG.sa_opaa_s,
        { title: LANG.sa_opaa_l },
        'antialias'
      ),
    ...
    $('lab-axis').onclick = () => {
      ui.lockX.checked =
        ui.lockY.checked =
        ui.lockZ.checked =
          !(ui.lockX.checked || ui.lockY.checked || ui.lockZ.checked);
    };
    ...
    DOC.oncontextmenu = (event) => {
      ...
      if (
        et.tagName === 'CANVAS' ||
        et.id === 'context-menu' ||
        et.classList.contains('draggable')
      ) {
    ...
    space.mouse.downSelect((int, event) => {
      ...
      if (api.feature.hover) {
        if (int) {
          return api.event.emit('mouse.hover.down', {
            int,
            point: int.point,
          });
        } else {
      ...
      if (
        int &&
        (event.ctrlKey || event.metaKey || api.feature.on_face_select)
      ) {
  ...
  function init_two() {
    ...
    // load CSS extensions
    if (SETUP.ss)
      SETUP.ss.forEach(function (style) {
    ...
    $('set-device').onclick = (ev) => {
      ev.stopPropagation();
      api.show.devices();
    };
    $('set-profs').onclick = (ev) => {
      ev.stopPropagation();
      api.conf.show();
    };
    $('set-tools').onclick = (ev) => {
      ev.stopPropagation();
      api.show.tools();
    };
    $('set-prefs').onclick = (ev) => {
      ev.stopPropagation();
      api.modal.show('prefs');
    };
    ui.acct.help.onclick = (ev) => {
      ev.stopPropagation();
      api.help.show();
    };
    ui.acct.don8.onclick = (ev) => {
      ev.stopPropagation();
      api.modal.show('don8');
    };
    ui.acct.mesh.onclick = (ev) => {
      ev.stopPropagation();
      WIN.location = '/mesh';
    };
    ui.acct.export.onclick = (ev) => {
      ev.stopPropagation();
      profileExport();
    };
    ...
    $('file-import').onclick = (ev) => {
      api.event.import(ev);
    };
    ui.func.slice.onclick = (ev) => {
      ev.stopPropagation();
      api.function.slice();
    };
    ui.func.preview.onclick = (ev) => {
      ev.stopPropagation();
      api.function.print();
    };
    ui.func.animate.onclick = (ev) => {
      ev.stopPropagation();
      api.function.animate();
    };
    ui.func.export.onclick = (ev) => {
      ev.stopPropagation();
      api.function.export();
    };
    ...
    let d = Math.PI / 180;
    $('rot_x_lt').onclick = () => {
      selection.rotate(-d * $('rot_x').value, 0, 0);
    };
    $('rot_x_gt').onclick = () => {
      selection.rotate(d * $('rot_x').value, 0, 0);
    };
    $('rot_y_lt').onclick = () => {
      selection.rotate(0, -d * $('rot_y').value, 0);
    };
    $('rot_y_gt').onclick = () => {
      selection.rotate(0, d * $('rot_y').value, 0);
    };
    $('rot_z_lt').onclick = () => {
      selection.rotate(0, 0, d * $('rot_z').value);
    };
    $('rot_z_gt').onclick = () => {
      selection.rotate(0, 0, -d * $('rot_z').value);
    };
    ...
    $('render-edges').onclick = () => {
      api.view.edges({ toggle: true });
      api.conf.save();
    };
    $('render-ghost').onclick = () => {
      api.view.wireframe(false, 0, api.view.is_arrange() ? 0.4 : 0.25);
    };
    $('render-wire').onclick = () => {
      api.view.wireframe(true, 0, api.space.is_dark() ? 0.25 : 0.5);
    };
    $('render-solid').onclick = () => {
      api.view.wireframe(false, 0, 1);
    };
    $('mesh-export-stl').onclick = () => {
      objectsExport('stl');
    };
    $('mesh-export-obj').onclick = () => {
      objectsExport('obj');
    };
    $('mesh-merge').onclick = selection.merge;
    $('context-duplicate').onclick = duplicateSelection;
    $('context-mirror').onclick = mirrorSelection;
    $('context-layflat').onclick = () => {
      api.event.emit('tool.mesh.lay-flat');
    };
    $('context-setfocus').onclick = () => {
      api.event.emit('tool.camera.focus', (ev) =>
        api.space.set_focus(undefined, ev.object.point)
      );
    };

    // ui.modal.onclick = api.modal.hide;
    ui.modalBox.onclick = (ev) => {
      ev.stopPropagation();
    };
    ...
    try {
      if (WIN.self !== WIN.top) $('top-sep').style.display = 'flex';
    } catch (e) {
      console.log(e);
    }
  ...
  function init_lang() {
    ...
    let lang = SETUP.ln
      ? SETUP.ln[0]
      : sdb.getItem('kiri-lang') || kiri.lang.get();
