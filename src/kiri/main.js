...
gapp.register('kiri.main', (root, exports) => {
  let { data, kiri, moto, noop } = root,
    ...
    FILES = (kiri.catalog = kiri.openFiles(
      new data.Index(SETUP.d ? SETUP.d[0] : 'kiri')
    )),
    clone = Object.clone,
    UI = {},
  ...
  assign(api, {
    ...
    event: assign(api.event, {
      ...
      emit(t, m, o) {
        return EVENT.publish(t, m, o)
      },
      bind(t, m, o) {
        return EVENT.bind(t, m, o)
      },
      alerts(clr) {
        api.alerts.update(clr)
      },
      import: loadFile,
    ...
    mode: assign(api.mode, {
      ...
      set_expert: noop,
      is_fdm() {
        return MODE === MODES.FDM
      },
      is_cam() {
        return MODE === MODES.CAM
      },
      is_sla() {
        return MODE === MODES.SLA
      },
      is_drag() {
        return MODE === MODES.DRAG
      },
      is_wedm() {
        return MODE === MODES.WEDM
      },
      is_wjet() {
        return MODE === MODES.WJET
      },
      is_laser() {
        return MODE === MODES.LASER
      },
      is_2d() {
        return (
          false ||
    ...
    show: assign(api.show, {
      alert() {
        return api.alerts.show(...arguments)
      },
    ...
    util: assign(api.util, {
      ...
      ui2rec() {
        api.conf.update_from(...arguments)
      },
      rec2ui() {
        api.conf.update_fields(...arguments)
      },
      b64enc(obj) {
      ...
      b64dec(obj) {
        return JSON.parse(new TextDecoder().decode(base64js.toByteArray(obj)))
      },
    }),
    view: assign(api.view, {
      ...
      set_slice() {
        api.view.set(VIEWS.SLICE)
      },
      set_preview() {
        api.view.set(VIEWS.PREVIEW)
      },
      set_animate() {
        api.view.set(VIEWS.ANIMATE)
      },
      is_arrange() {
        return viewMode === VIEWS.ARRANGE
      },
      is_slice() {
        return viewMode === VIEWS.SLICE
      },
      is_preview() {
        return viewMode === VIEWS.PREVIEW
      },
      is_animate() {
        return viewMode === VIEWS.ANIMATE
      },
  ...
  function setWireframe(bool, color, opacity) {
    api.widgets.each(function (w) {
      w.setWireframe(bool, color, opacity)
    })
    SPACE.update()
  }
  ...
  function loadImageDialog(image, name, force) {
    if (!force && image.byteLength > 2500000) {
      return UC.confirm(
        'Large images may fail to import<br>Consider resizing under 1000 x 1000<br>Proceed with import?'
      ).then((ok) => {
        if (ok) {
  ...
  function loadImageConvert(res, name) {
    ...
    $('mod-any').innerHTML =
      `<img id="xsrc" src="${url}"><canvas id="xdst"></canvas>`
  ...
  function saveWorkspace(quiet) {
    ...
    FILES.deleteFilter(
      (key) => newWidgets.indexOf(key.substring(8)) < 0,
      'ws-save-',
      'ws-savf'
    )
    if (!quiet) {
  ...
  function setViewMode(mode) {
    ...
    ;['view-arrange', 'act-slice', 'act-preview', 'act-animate'].forEach(
      (el) => {
  ...
  function downloadBlob(data, filename) {
    let url = WIN.URL.createObjectURL(
      new Blob([data], { type: 'octet/stream' })
    )
    $('mod-any').innerHTML =
      `<a id="_dexport_" href="${url}" download="${filename}">x</a>`
    $('_dexport_').click()
  }
