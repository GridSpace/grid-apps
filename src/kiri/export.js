...
gapp.register('kiri.export', [], (root, exports) => {
  ...
  let printSeq =
    parseInt(local['kiri-print-seq'] || local['print-seq'] || '0') + 1;
  ...
  function exportFile(options) {
    ...
    switch (mode) {
      ...
      case 'DRAG':
      case 'LASER':
        return callExportLaser(options, names);
      case 'FDM':
        return callExport(options, mode, names);
      case 'CAM':
        return callExport(options, mode, names);
      case 'SLA':
        return callExportSLA(options, names);
    }
  }
  ...
  function exportGCodeDialog(gcode, sections, info, names) {
    ...
    function getBlob() {
      return new Blob([gcode], { type: 'application/octet-stream' });
    }
    ...
    function calcWeight() {
      try {
        ...
        info.weight = (
          (Math.PI *
            util.sqr(info.settings.device.extruders[0].extFilament / 2) *
            info.distance *
    ...
    download3MF.onclick = function () {
      gen3mf((zip) =>
        api.util.download(zip, `${$('print-filename').value}.3mf`)
      );
    };
    ...
    if (preview && gcode)
      $('code-preview-textarea').value = gcode.substring(0, 65535);
  }
});
