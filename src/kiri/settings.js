...
gapp.register('kiri.settings', (root, exports) => {
  ...
  function loadSettings(evt, named) {
    let mode = settings.mode,
      name = evt
        ? evt.target.getAttribute('load')
        : named || api.process.get() || 'default',
  ...
  function updateSettings(opt = {}) {
    ...
    let same = true;
    if (compare)
      for (let [key, val] of Object.entries(compare).filter(
        (v) => v[0] !== 'processName'
      )) {
  ...
  function updateExtruderFields(device) {
    ...
    if (device.extruders && device.extruders[device.internal]) {
      ...
      ui.extruder.innerHTML = `${LANG.dv_gr_ext}<label class='grow'></label>${device.internal + 1} of ${device.extruders.length}`;
      ...
      ui.extDel.onclick = function () {
        ...
        device.internal = Math.min(
          device.internal,
          device.extruders.length - 1
        );
        updateExtruderFields(device);
      };
  ...
  function updateFieldsFromRange() {
    ...
    for (let range of getOverlappingRanges(
      api.var.layer_lo,
      api.var.layer_hi
    )) {
  ...
  function settingsExport(opts = {}) {
    ...
    const work = opts.work
      ? kiri.codec.encode(widgets, { _json_: true })
      : undefined;
  ...
  function settingsImport(data, ask) {
    ...
    if (typeof data === 'string') {
      try {
        data =
          data.charAt(0) === '{' ? JSON.parse(data) : api.util.b64dec(data);
      } catch (e) {
    ...
    let isProcess =
      data.process && data.version && data.time && data.mode && data.name;
  ...
  function settingsImportUrl(url, ask) {
    ...
    fetch(url)
      .then((r) => (kmz ? r.arrayBuffer() : r.text()))
      .then((a) => {
  ...
  Object.assign(api.settings, {
    ...
    prof() {
      return settings.sproc[settings.mode];
    },
    sync: {
      async get() {},
      async put() {},
