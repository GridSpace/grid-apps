...
gapp.register('kiri.ui', [], (root, exports) => {
  ...
  function confirm(message, buttons, input, opt = {}) {
    return new Promise((resolve, reject) => {
      ...
      if (Array.isArray(input)) {
        ...
        html.append(
          `<div><textarea rows="15" cols="40" class="grow" type="text" spellcheck="false" id="${iid}"></textarea></div>`
        );
      } else if (input !== undefined) {
        iid = `confirm-input-${rnd}`;
        html.append(
          `<div><input class="grow" type="text" spellcheck="false" id="${iid}"/></div>`
        );
      }
  ...
  function newInput(label, opt = {}) {
    let row = newDiv(opt),
      ...
      height = opt.height || 0,
      ip =
        height > 1 ? DOC.createElement('textarea') : DOC.createElement('input'),
