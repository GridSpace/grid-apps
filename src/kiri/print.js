...
gapp.register('kiri.print', [], (root, evets) => {
  ...
  class Print {
    ...
    addOutput(array, point, emit, speed, tool, type) {
      ...
      this.lastEmit = emit;
      this.lastOut = lastOut = new Output(
        point,
        emit,
        speed,
        tool,
    ...
    polyPrintPath(poly, startPoint, output, options = {}) {
      ...
      if (perimeter < process.outputShortPoly) {
        printSpeed =
          minSpeed +
          (printSpeed - minSpeed) * (perimeter / process.outputShortPoly);
      }

      poly.forEachPoint(
        (point, pos, points, count) => {
          if (first) {
            if (options.onfirst) {
    ...
    constReplace(str, consts, start, pad, short) {
      ...
      if (cs >= 0 && ce > cs) {
        ...
        return this.constReplace(
          nustr,
          consts,
          ce + 1 + (nustr.length - str.length),
          pad,
          short
        );
      } else {
    ...
    parseGCode(gcode, offset, progress, done, opts = {}) {
      ...
      function G0G1(g0, line) {
        ...
        if (pos.A) {
          let ip = new THREE.Vector3(pos.X, pos.Y, pos.Z).applyAxisAngle(
            XAXIS,
            -pos.A * DEG2RAD
          );
          point.x = ip.x;
        ...
        const moving =
          g0 || (fdm && (pos.E <= 0 || !(axes.X || axes.Y || axes.Z)));
