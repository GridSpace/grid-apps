...
gapp.register('kiri.devices', (root, exports) => {
      ...
      if (code.profiles) {
        for (let profile of code.profiles) {
          ...
          if (newdev && !current.devproc[devicename]) {
            console.log(
              'setting default profile for new device',
              devicename,
              'to',
              profname
            )
  ...
  function renderDevices(devices) {
    let selected = api.device.get() || devices[0],
      ...
      devs = api.conf.get().devices,
      dfilter =
        typeof features.device_filter === 'function'
          ? features.device_filter
          : undefined
    ...
    for (let local in devs) {
      ...
      if (
        dev.mode ? dev.mode === api.mode.get() : fdmCode ? fdmMode : !fdmMode
      ) {
        devices.push(local)
      }
    }
