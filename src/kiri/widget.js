  ...
  class Widget {
    ...
    loadVertices(data, options = { index: false }) {
      ...
      if (vertices.buffer) {
        if (hasSharedArrays && vertices.buffer instanceof sharedArrayClass) {
          ...
          let newvert = new Float32Array(
            this.#newBuffer(vertices.buffer.byteLength)
          )
    ...
    getTabVertices() {
      ...
      if (this.anno && this.anno.tab) {
        for (let tab of this.anno.tab) {
          ...
          box.applyMatrix4(new THREE.Matrix4().makeRotationFromQuaternion(quat))
    ...
    setWireframe(set, color, opacity) {
      ...
      if (set) {
        ...
        let wire = (widget.wire = new THREE.Mesh(
          mesh.geometry.shallowClone(),
          mat
        ))
        mesh.add(wire)
      }
