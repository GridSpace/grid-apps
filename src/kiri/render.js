    ...
    levels.forEach((level, index) => {
      ...
      level.forEach((out) => {
        ...
        if (outPoint && outPoint.a !== undefined) {
          ...
          let p2 = new THREE.Vector3(point.x, point.y, point.z).applyAxisAngle(
            XAXIS,
            point.a * DEG2RAD
          )
        ...
        if (lastOut) {
          ...
          if (out.emit) {
            if (
              !lastOut.emit ||
              (ckspeed && out.speed !== lastOut.speed) ||
              lastEnd
            ) {
      ...
      if (lasers.length) {
        for (let poly of lasers)
          output
            .setLayer(
              'power',
              { line: 0x000055, face: 0x0000ff, opacity: 0.5 },
              true
            )
            .addPolys([poly], {
              thin: true,
      ...
      if (retracts.length) {
        output
          .setLayer(
            'retract',
            { line: 0x550000, face: 0xff0000, opacity: 0.5 },
            true
          )
          .addAreas(
            retracts.map((point) => {
              return newPolygon()
                .centerCircle(point, 0.2, 5)
                .setZ(point.z + 0.01)
            }),
            { outline: true }
          )
      ...
      if (engages.length) {
        output
          .setLayer(
            'engage',
            { line: 0x005500, face: 0x00ff00, opacity: 0.5 },
            true
          )
          .addAreas(
            engages.map((point) => {
              return newPolygon()
                .centerCircle(point, 0.2, 7)
                .setZ(point.z + 0.02)
            }),
            { outline: true }
          )
      ...
      for (let array of Object.values(prints)) {
        for (let poly of array) {
          ...
          output.setLayer(action, printOpt).addPolys(
            [poly],
            thin
              ? { thin, z: opts.z, color: poly.color }
              : flat
                ? {
                    flat,
                    z: pz,
