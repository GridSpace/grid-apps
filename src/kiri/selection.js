...
gapp.register('kiri.selection', (root, exports) => {
  ...
  function for_meshes(fn) {
    selectedMeshes.slice().forEach((mesh) => {
      fn(mesh);
    });
  }
  ...
  const selection = Object.assign(api.selection, {
    ...
    add(w) {
      selectedMeshes.addOnce(w.mesh);
    },
    delete() {
      api.platform.delete(widgets());
    },
    remove(w) {
      return selectedMeshes.remove(w.mesh);
    },
    count() {
      return selectedMeshes.length;
    },
    contains(w) {
      return selectedMeshes.indexOf(w.mesh) >= 0;
    },
    enable() {
      setDisabled(false);
    },
    ...
    opacity() {
      api.widgets.opacity(...arguments);
    },
    meshes() {
      return selectedMeshes.slice();
    },
  });
});
