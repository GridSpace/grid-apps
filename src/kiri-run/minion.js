  ...
  const funcs = (self.minion = {
    ...
    clip: (data, seq) => {
      ...
      let lines = data.lines.map((array) => {
        return codec.decodePointArray2D(array, data.z, (X, Y) => {
          return { X: X * M, Y: Y * M }
        })
      })
      ...
      if (
        ...
        for (let node of ctre.m_AllPolys) {
          clips.push(
            codec.encode(POLY.fromClipperNode(node, data.z), state.zeros)
          )
        }
      }
    ...
    sliceZ: (data, seq) => {
      ...
      while (i < points.length) {
        realp[p++] = base
          .newPoint(points[i++], points[i++], points[i++])
          .round(3)
      }
      let state = { zero: [] }
      let output = []
