...
gapp.register('kiri-run.worker', [], (root, exports) => {
  ...
  const minwork = (kiri.minions = {
    ...
    start() {
      ...
      console.log(
        `kiri | init pool | ${gapp.version || 'rogue'} | ${concurrent + 1}`
      );
    },
    ...
    union(polys, minarea) {
      return new Promise((resolve, reject) => {
        if (
          concurrent < 2 ||
          polys.length < concurrent * 2 ||
          POLY.points(polys) < concurrent * 50
        ) {
        ...
        minwork.queue(
          {
            cmd: 'clip',
            polys: codec.encode(
              POLY.flatten(polys).map((poly) =>
                codec.encodePointArray2D(poly.points, state)
              ),
              state
            ),
            lines: codec.encode(
              lines.map((array) => codec.encodePointArray2D(array, state)),
              state
            ),
            z: slice.z,
          },
    ...
    kiri.worker =
      {
        ...
        prepare(data, send) {
          ...
          if (!(driver && driver.prepare)) {
            return console.log({
              invalid_print_driver: mode,
              driver,
            });
          }

          driver
            .prepare(widgets, settings, (progress, message, layer) => {
              const state = { zeros: [] };
              const emit = {
                progress,
                message,
                layer: layer ? layer.encode(state) : undefined,
              };
              send.data(emit, state.zeros);
            })
            .then(() => {
              const unitScale =
                settings.controller.units === 'in' ? 1 / 25.4 : 1;
        ...
        parse(args, send) {
          ...
          const print = (current.print = kiri.newPrint(
            settings,
            Object.values(wcache)
          ));
          const tools = device.extruders;
          const mode = settings.mode;
          const thin =
            settings.controller.lineType === 'line' || mode !== 'FDM';
          const flat =
            settings.controller.lineType === 'flat' && mode === 'FDM';
          const parsed = print.parseGCode(
            code,
            offset,
            (progress) => {
        ...
        image2mesh(info, send) {
          ...
          load.PNG.parse(info.png, {
            ...
            progress(progress) {
              ...
              send.done({ vertices }, [vertices.buffer]);
            },
          });
        },

        gerber2mesh(data, send) {
          const vertices = load.GBR.toMesh(data, {
            progress(pct) {
