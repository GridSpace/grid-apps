...
const interact = {
    ...
    handleMouseUp(intersection, event, allIntersections) {
        ...
        if (!intersection) {
            // Ignore non-viewport UI clicks (toolbar/tree/dialog) so selection
            // survives command-button workflows like "select plane -> Sketch".
    ...
    /**
     * Resolve primary sketch target from hover first, then selection fallback.
     */
    ...
    /**
     * Fallback sketch target: single selected plane.
     */
    ...
    /**
     * Resolve sketch target from hovered intersection.
     * Extension hook: object.userData.sketchTargetResolver({ intersection, object })
     */
    resolveSketchTarget(intersection) {
    ...
    sketchTargetFromFace(intersection, object) {
    ...
    getFaceXAxisWorld(intersection, object, normal) {
    ...
    projectAxisOnPlane(axis, normal) {
