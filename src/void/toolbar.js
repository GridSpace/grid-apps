    ...
    async exportSolids(format = 'obj') {
        ...
        if (ext === 'obj') {
            ...
            if (zipPerSolid) {
                ...
                try {
                    // Guard against very large one-shot allocations in STL encoder.
                    let triCount = 0;
                    for (const rec of recs) {
                        triCount += Math.floor((rec?.varr?.length || 0) / 9);
                    }
                    const bytes = 84 + triCount * 50;
                    const maxBytes = 512 * 1024 * 1024; // 512 MB safety cap
                    if (bytes > maxBytes) {
                        throw new RangeError(`STL export too large (${Math.round(bytes / (1024 * 1024))}MB)`);
                    }
                    const data = stlEncode(recs, 'Generated by Void:Form');
                    this.downloadExport(data, filename, 'application/sla');
                } catch (error) {
                    console.warn('void.export.stl.failed', error);
                    window.alert('STL export failed (size/memory). Try "zip with file per solid" or export OBJ/3MF.');
                    return;
                }
            }
        } else if (ext === '3mf') {
