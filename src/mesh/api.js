...
gapp.register('mesh.api', [], (root, exports) => {
  ...
  const selection = {
    ...
    update() {
      ...
      let mgsel = selected
        .filter((s) => s instanceof mesh.model)
        .map((m) => m.group);
      selected = selected
        .filter((sel) => !mgsel.contains(sel))
        .filter((v) => v);
      // highlight selected
      ...
      api.modal.dialog({
        ...
        body: [
          h.div({ class: 'additem' }, [
            ...
            h.label('chamfer top'),
            h.input({
              value: opt.chamfer_top || 0,
              size: 5,
              id: '_chamtop',
            }),
            h.label('chamfer bottom'),
            h.input({
              value: opt.chamfer_bottom || 0,
              size: 5,
              id: '_chambot',
            }),
            h.button({
        ...
        sketch.selection.ifcan((sketch, items) => {
          items
            ...
            .forEach((item) => {
              Object.assign(
                item,
                newPolygon().fromObject(item).flip('x').toObject()
              );
            });
          sketch.render();
        });
        ...
        sketch.selection.ifcan((sketch, items) => {
          items
            ...
            .forEach((item) => {
              Object.assign(
                item,
                newPolygon().fromObject(item).flip('y').toObject()
              );
            });
          sketch.render();
        });
        ...
        function doit(sketch, items, angle) {
          items
            ...
            .forEach((item) => {
              Object.assign(
                item,
                newPolygon().fromObject(item).rotate(-angle).toObject()
              );
            });
          sketch.render();
        }
    ...
    cylinder() {
      function gencyl(opt = {}) {
        ...
        if (diameter && height) {
          ...
          log(
            `add cylinder faces=${facets} height=${height} diameter=${diameter}`
          );
        ...
        body: [
          h.div({ class: 'additem' }, [
            ...
            h.button({
              _: 'create',
              onclick() {
                const { _diameter, _facets, _height, _chamfer, _bore } =
                  api.modal.bound;
                gencyl({
      ...
      api.modal.dialog({
        ...
        body: [
          h.div({ class: 'additem' }, [
            ...
            h.label('z height'),
            h.input({
              value: last.height || 15,
              size: 5,
              id: '_height',
            }),
            h.label({
              _: 'chamfer',
              title: 'negative values apply chamfer to the bottom only',
            }),
            h.input({
              value: last.chamfer || 0,
              size: 5,
              id: '_chamfer',
            }),
            h.hr(),
      ...
      api.modal.dialog({
        ...
        body: [
          h.div({ class: 'additem' }, [
            h.label('height'),
            h.input({
              value: last.height || 25,
              size: 5,
              id: '_height',
            }),
            h.label('radius'),
            h.input({
              value: last.radius || 10,
              size: 5,
              id: '_radius',
            }),
            h.label('turns'),
  ...
  let file = {
    ...
    export() {
      let recs = selection.models().map((m) => {
        return {
          id: m.id,
          matrix: m.matrix,
          file: m.file,
  ...
  const tool = {
    ...
    difference(models) {
      ...
      worker
        .model_difference(models.map((m) => m.id))
        .then((data) => {
          let group = api.group
            .new([
              new mesh.model({
    ...
    intersect(models) {
      ...
      worker
        .model_intersect(models.map((m) => m.id))
        .then((data) => {
          let group = api.group
            .new([
              new mesh.model({
      ...
      for (let m of models) {
        let p = worker.model_analyze({ id: m.id, opt }).then((data) => {
          ...
          let nm = areas
            ...
            .map((nm) =>
              nm.applyMatrix4(mcore.clone().multiply(m.mesh.matrixWorld))
            );
          if (nm.length) {
  ...
  const api = exports({
    ...
    wireframe(
      state = { toggle: true },
      opt = { opacity: prefs.map.wireframe.opacity }
    ) {
