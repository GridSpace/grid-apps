  ...
  mesh.tool = class MeshTool {
    ...
    generateFaces(vertices, opt = {}) {
      ...
      for (let i = 0, l = vertices.length; i < l; i += 3) {
        ...
        let y = vertices[i + 1]
        let z = vertices[i + 2]
        let key = [(x * prec) | 0, (y * prec) | 0, (z * prec) | 0].join('')
        let vpos = hash[key]
        if (vpos === undefined) {
    ...
    patch(opt = { merge: true }) {
      ...
      function getLine(v1, v2) {
        let s1 =
          vertices[v1] + vertices[v1 + 1] * 1000 + vertices[v1 + 2] * 100000
        let s2 =
          vertices[v2] + vertices[v2 + 1] * 1000 + vertices[v2 + 2] * 100000
      ...
      for (let i = 0, l = edges.length; i < l; i++) {
        ...
        while (true) {
          ...
          if (adjacent.length === 0) {
            ...
            line = adjacent[0].v1 === line.v2 ? adjacent[0] : adjacent[1]
          } else {
      ...
      function emitLoop(loop) {
        ...
        for (let i = 0; i < loop1.length; i++) {
          ...
          if (lastPoint >= 0) {
            dx = Math.max(
              dx,
              Math.abs(vertices[lastPoint] - vertices[nextPoint])
            )
            dy = Math.max(
              dy,
              Math.abs(vertices[lastPoint + 1] - vertices[nextPoint + 1])
            )
            dz = Math.max(
              dz,
              Math.abs(vertices[lastPoint + 2] - vertices[nextPoint + 2])
            )
          }
          lastPoint = nextPoint
        }
    ...
    generateGear(numTeeth, module, pressureAngle, offset) {
      ...
      // involute gear maker
      {
        ...
        let points = [
          // [x,y] points for a single gear tooth
          polar(r, -pi / numTeeth),
    ...
    generateThreads(height, radius, turns, depth, steps, taper) {
      ...
      for (let turn = 0; turn < turns; turn++) {
        for (let step = 0; step < steps; step++) {
          ...
          let rm = radius - depth
          let rx =
            true && taper && iter < mute_min
              ? rm + (iter / mute_min) * depth
              : taper && iter > mute_max
                ? rm + ((iter_max - iter) / mute_min) * depth
                : radius
          let rxt =
            true && taper && itern < mute_min
              ? rm + (itern / mute_min) * depth
              : taper && itern > mute_max
                ? rm + ((iter_max - itern) / mute_min) * depth
                : radius
