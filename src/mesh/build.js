...
gapp.register('mesh.build', [], (root, exports) => {
  ...
  util.download = (data, filename = 'mesh-data') => {
    ...
    $('download').innerHTML =
      `<a id="_data_export_" href="${url}" download="${filename}">x</a>`;
    $('_data_export_').click();
  };
  ...
  const log = (api.log = {
    ...
    emit() {
      ...
      while (
        !log.pinned &&
        data.length &&
        (data.length > lines || now - data[0].time > age)
      ) {
        data.shift();
      }
      return render();
    ...
    render() {
      ...
      bind(
        area,
        log.data.map((rec) => div({ _: rec.text }))
      );
  ...
  api.settings = function () {
    ...
    const set2 = div([
      ...
      input({
        type: 'text',
        size: 5,
        value: parseFloat(wireframe.opacity),
      ...
      input({
        type: 'text',
        size: 5,
        value: parseFloat(wireframe.fog),
    ...
    const set3 = div([
      ...
      input({
        type: 'text',
        size: 6,
        value: parseFloat(normals.length),
      ...
      input({
        ...
        onchange: (ev) =>
          call.set_normals_color(util.fromHexRGB(ev.target.value)),
      }),
    ]);
    ...
    const set4 = div([
      ...
      input({
        type: 'text',
        size: 5,
        value: parseFloat(surface.radians),
      ...
      input({
        type: 'text',
        size: 5,
        value: parseFloat(surface.radius || 0.2),
    ...
    let srr = () => {
      api.sketch.list().forEach((s) => s.render());
    };
    const set7 = div([
      ...
      input({
        ...
        onchange: (ev) =>
          prefs.save(
            ((sketch.open_close = !sketch.open_close) || true) && srr()
          ),
      ...
      input({
        type: 'text',
        size: 5,
        value: parseFloat(sketch.open_width || 1),
        onchange: (ev) =>
          prefs.save(
            (sketch.open_width = parseFloat(ev.target.value)) && srr()
          ),
      }),
      label('type'),
      h.select(
        {
          type: 'text',
          value: sketch.open_type,
          onchange: (ev) =>
            prefs.save((sketch.open_type = ev.target.value) && srr()),
        },
        [
          h.option({
            _: 'square',
    ...
    modal.show(
      'settings',
      div({ class: 'settings' }, [set1, set2, set3, set4, set5, set6, set7])
    );
  };
  ...
  function menu_item(text, fn, short, id) {
    if (short) {
      ...
      short = div(
        { class: 'short' },
        short.map((s) => (s.startsWith('bi-') ? bicon(s) : div(s)))
      );
    }
    return div(
      { id, onclick: fn },
      [div({ _: text, class: 'grow' }), short].filter((o) => o)
    );
  }
  ...
  function ui_build() {
    ...
    let eye_closed = FontAwesome.icon({
      prefix: 'fas',
      iconName: 'eye-slash',
    }).html[0];
      ...
      div({ class: 'menu' }, [
        ...
        div({ class: 'menu-items' }, [
          input({
            ...
            multiple: true,
            onchange(evt) {
              broker.send.load_files(evt.target.files);
            },
          }),
    ...
    bind($('modal_page'), [
      div({ id: 'modal_frame' }, [
        ...
        div({ id: 'modal' }, [div({ _: 'this is a modal test' })]),
      ]),
    ...
    function tool_item(icon, help, fn) {
      return div({ onclick: fn, class: 'tool' }, [
        bicon(icon),
        div([label(help)]),
      ]);
    }
    ...
    bind(
      ...
      div([
        ...
        tool_item(
          ...
          'Flip Horizontal',
          api.sketch.arrange.fliph
        ),
        tool_item(
          'bi-symmetry-horizontal',
          'Flip Vertical',
          api.sketch.arrange.flipv
        ),
    ...
    function grid3(v1, v2, side = ['pos', 'rot'], top = ['X', 'Y', 'Z'], root) {
      ...
      return div({ class: 'grid' }, [
        ...
        div({
          _: X,
          class: 'top noselect',
          id: root ? [root, l, X] : und,
        }),
        div({
          _: Y,
          class: 'top noselect',
          id: root ? [root, l, Y] : und,
        }),
        div({
          _: Z,
          class: 'top noselect',
          id: root ? [root, l, Z] : und,
        }),
    ...
    function update_selector() {
      ...
      // map groups to divs
      let groups = api.group.list().map((g) =>
        div([
          h.button({
            _: g.name || `group`,
            title: g.id,
      ...
      if (sketches.length) {
        let sg = div([
          h.button({ _: 'sketch', class: ['group'] }),
          sketches.map((sketch) =>
            div({ class: 'models' }, [
              h.button({
                _: sketch.file || sketch.id,
    ...
    function update_selection() {
      ...
      if (sketch) {
                ...
                if (item.type === 'polygon') {
                  ...
                  return [
                    sblock(
                      ...
                      grid1(
                        [
                          item.points || sel_sobj.poly.length,
                          item.rotation || 0,
                        ],
                        ['points', 'rotation'],
                        'Value',
        ...
        let bound = bind(selectlist, [...si_vars, ...sk_vars]);
        for (let axis of ['x', 'y', 'z']) {
          let el = bound[`sketch_val_${axis.toUpperCase()}_center`];
          el.onclick = field_edit(
            `${axis} center`,
            (nval, oval) => {
      ...
      let bound = bind(selectlist, [...h_grp, ...h_ara, ...h_msh, ...h_ops]);
      ...
      group_val_X_pos.onclick = field_edit(
        'x position',
        (group, val) => {
      ...
      group_val_Y_pos.onclick = field_edit(
        'y position',
        (group, val) => {
      ...
      group_val_Z_pos.onclick = field_edit(
        'z position',
        (group, val) => {
