...
gapp.register('mesh.model', [], (root, exports) => {
  ...
  mesh.model = class MeshModel extends mesh.object {
    ...
    load(vertices) {
      ...
      let meh = (this.mesh = new Mesh(geo, [this.mats.normal, this.mats.face]));
    ...
    wireframe(bool, opt = {}) {
      ...
      if (bool === 'edges') {
        ...
        this._wire = new Mesh(
          this.mesh.geometry.shallowClone(),
          materials.wireframe
        );
        this.mesh.add(this._wire);
        this.opacity({ temp: opt.opacity || 0.15 });
    ...
    selectionToSketch() {
      ...
      let bounds = new Box3().setFromArray(
        polys
          .map((p) => p.points.map((p) => p.toArray()))
          .flat()
          .flat()
      );
      let center = bounds.getCenter(new Vector3());
      polys.forEach((p) =>
        p.move({ x: -center.x, y: -center.y, z: -center.z })
      );
    ...
    selectionToRotationPolys() {
      ...
      if (selverts?.length) {
        ...
        let rotato = new THREE.Quaternion().setFromUnitVectors(
          norm,
          targetNorm
        );
