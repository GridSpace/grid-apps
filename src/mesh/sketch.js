...
gapp.register('mesh.sketch', [], (root, exports) => {
  ...
  const { MeshBasicMaterial, LineBasicMaterial, LineSegments, DoubleSide } =
    THREE;
  const { PlaneGeometry, EdgesGeometry, SphereGeometry, Vector3, Mesh, Group } =
    THREE;
  ...
  mesh.sketch = class MeshSketch extends mesh.object {
    constructor(opt = {}) {
      ...
      const outline = (this.outline = new LineSegments(
        outlineGeometry,
        outlineMaterial
      ));
    ...
    update() {
      let { group, plane, outline, center, handles, corners, normal, scale } =
        this;
    ...
    get meshes() {
      return this.group.children
        .filter((c) => {
          return c.sketch || (c.sketch_item && c.sketch_item.selected)
            ? c
            : undefined;
        })
        .reverse();
    ...
    get selection() {
      ...
      return {
        ...
        mesh_items() {
          return sketch.group.children.filter(
            (c) => c.sketch_item && c.sketch_item.selected
          );
        },
    ...
    get boolean() {
      ...
      return {
        union() {
          ...
          if (items.length < 2)
            return log('operation requires at least 2 items');
          let polys = items.map((i) => i.sketch_item.poly);
          let union = POLYS.union(polys, 0, true);
          sketch.selection.delete();
          sketch.arrange.group(
            union.map((poly) => sketch.add.polygon({ poly }))
          );
        },
        ...
        nest() {
          ...
          if (items.length < 2)
            return log('operation requires at least 2 items');
          let polys = items.map((si) =>
            si.poly.annotate({ group: si.group }).clone(true, ['group'])
          );
        ...
        evenodd() {
          ...
          let even = POLYS.nest(
            POLYS.flatten(polys.clone(true, ['group']), [], true)
          );
          let odd = POLYS.nest(
            POLYS.flatten(even.clone(true, ['group']), [], true).filter(
              (p) => p.depth > 0
            )
          );
          sketch.selection.delete();
    ...
    drag(opt = {}) {
      ...
      if (start) {
        ...
        let pos =
          handle >= 0 ? this.handle_pos(handle) : (item?.center ?? center);
        let end = {
          x: start.x + offset.x,
          y: start.y + offset.y,
          z: start.z + offset.z,
  ...
  class SketchItem {
    ...
    update() {
      ...
      if (type === 'circle') {
        ...
        poly = newPolygon()
          .centerCircle(center, radius, points)
          .annotate({ item })
