...
gapp.register('kiri-mode.cam.animate2', [], (root) => {
  ...
  kiri.load(() => {
    ...
    function meshUpdate(id, ind, pos, ilen, plen) {
      ...
      geo.setAttribute(
        'position',
        new THREE.BufferAttribute(mesh.pos.subarray(0, plen * 3), 3)
      );
  ...
  kiri.load(() => {
    ...
    function renderPath(send) {
      ...
      if (!next) {
        console.log(
          ...
          ((Date.now() - startTime) / 1000).round(2)
        );
        animating = false;
        renderPath(send);
        return;
      ...
      if (last) {
        ...
        const sd = Math.sqrt(
          mx * mx + my * my + Math.min(1, mz * mz) + dr * dr
        );
        const moves = [];
    ...
    function renderUpdate(send) {
      ...
      if (stockIndexMsg) {
        ...
        for (let slice of stockSlices) {
          send.data({
            mesh_index: { id: slice.id, index: -stockIndex },
          });
        }
        stockIndexMsg = false;
      }
    ...
    function toolUpdate(toolid, send) {
      ...
      if (tool.isBallMill()) {
        ...
        mesh = cylinder(slen, srad, srad, 20, true)
          .translate(0, 0, slen / 2)
          .add(cylinder(flen, trad, frad, 20, true).translate(0, 0, -flen / 2));
      } else {
        mesh = cylinder(tlen, frad, frad, 20, true).add(
          cylinder(slen, srad, srad, 20, true).translate(0, 0, flen / 2)
        );
      }
