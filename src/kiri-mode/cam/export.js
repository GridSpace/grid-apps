  ...
  CAM.export = function (print, online) {
      ...
      if (line) {
        ...
        if (rewrite_comments) {
          ...
          if (spos >= 0) {
            ...
            const right =
              '(' +
              line.substring(spos).replace(/; /g, '').replace(/;/g, '') +
              ')';
    ...
    function moveTo(out, opt = {}) {
      ...
      if (
        spindleMax &&
        newSpindle &&
        (changeTool || newSpindle !== spindle || opt.newOp)
      ) {
        spindle = newSpindle;
        if (!laserOp) {
          if (spindle > 0) {
      ...
      if (dx || newpos.x !== pos.x) {
        ...
        nl.append(space)
          .append(axis.X)
          .append(add0((consts.pos_x = pos.x * factor)));
      }
      if (dy || newpos.y !== pos.y) {
        ...
        nl.append(space)
          .append(axis.Y)
          .append(add0((consts.pos_y = pos.y * factor)));
      }
      if (dz || newpos.z !== pos.z) {
        ...
        nl.append(space)
          .append(axis.Z)
          .append(add0((consts.pos_z = pos.z * factor)));
      }
      if (da) {
        pos.a = newpos.a;
        nl.append(space)
          .append(axis.A)
          .append(add0((consts.pos_a = pos.a * -factor)));
      }
      if (newFeed) {
        pos.f = feed;
        nl.append(space)
          .append(axis.F)
          .append(add0((consts.feed = feed * factor), true));
      }
    ...
    if (!stripComments) {
      ...
      filterEmit(
        ['; Bed left:{left} right:{right} top:{top} bottom:{bottom}'],
        consts
      );
      append(
        `; Stock X:${stock.x.round(2)} Y:${stock.y.round(2)} Z:${stock.z.round(2)}`
      );
    ...
    if (!stripComments) {
      ...
      Object.keys(toolz)
        ...
        .forEach((tn) => {
          ...
          append(
            `; tool#=${number} flute=${flute_diam} len=${flute_len} unit=${metric ? 'metric' : 'imperial'}`
          );
        });
    }
    // emit gcode preamble
    ...
    if (mode && !stripComments) {
      append(
        '; ending ' + mode.type + ' op after ' + Math.round(time) + ' seconds'
      );
    }
