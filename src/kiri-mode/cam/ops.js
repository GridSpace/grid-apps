...
gapp.register('kiri-mode.cam.ops', (root, exports) => {
    ...
    async slice(progress) {
      ...
      if (workarea.top_z > workarea.top_part) {
        ...
        let inset = roughStock
          ...
          : POLY.offset(
              shadow,
              roughIn ? step : step + roughLeave + toolDiam / 2
            );
      ...
      let cnt = 0;
      let tot = 0;
      await slicer.slice(indices, {
        each: (data) => {
          shadow = unsafe
            ? data.tops
            : POLY.union(shadow.slice().appendAll(data.tops), 0.01, true);
      ...
      slices.forEach((slice, index) => {
        ...
        offset =
          POLY.offset(
            nest,
            [-(toolDiam / 2 + roughLeave), -toolDiam * op.step],
            {
        ...
        if (!roughIn && !roughStock) {
          const outside = POLY.offset(
            shadow.clone(),
            toolDiam / 2 + roughLeave,
            { z: slice.z }
          );
          if (outside) {
        ...
        if (process.camStockClipTo && stock.x && stock.y && stock.center) {
          let rect = newPolygon().centerRectangle(
            stock.center,
            stock.x,
            stock.y
          );
  ...
  class OpOutline extends CamOp {
    ...
    async slice(progress) {
      ...
      let toolDiam = (this.toolDiam = new CAM.Tool(
        settings,
        op.tool
      ).fluteDiameter());
      updateToolDiams(toolDiam);
      ...
      if (op.outside && !op.inside) {
        ...
        await slicer.slice(indices, {
          each: (data) => {
            ...
            data.shadow = trueShadow
              ? CAM.shadowAt(widget, data.z)
              : shadow.clone(true);
            data.slice.shadow = data.shadow;
          ...
          if (op.wide) {
            ...
            for (let c = op.steps || 1; c > 0; c--) {
              offset.slice().forEach((op) => {
                ...
                wideCut.forEach((cut) => {
                  //set order of cuts when wide
                  cut.order = c;
                  if (cut.inner)
                    cut.inner.forEach((inn) => {
                      inn.order = c;
                    });
                });
    ...
    prepare(ops, progress) {
      ...
      for (let slice of sliceOut) {
        ...
        if (depthFirst) {
          ...
          Object.entries(orderSplit) //split the polys by order
            ...
            .forEach(([order, orderPolys]) => {
              // emit based on closest for each order
      ...
      if (depthFirst) {
        let flatLevels = depthData
          .map((level) => {
            return POLY.flatten(level.clone(true), [], true).filter(
              (p) => !(p.depth = 0)
            );
          })
          .filter((l) => l.length > 0);
      ...
      function clearZ(polys, z, down) {
        ...
        for (let poly of nested) {
          for (let z of zs) {
            ...
            if (tabs) {
              slice.camLines = cutTabs(
                tabs,
                POLY.flatten(slice.camLines, null, true),
                z
              );
            } else {
      ...
      switch (op.mode) {
        ...
        case 'clear':
          ...
          for (let poly of polys) {
            ...
            if (offover) {
              let pnew = POLY.offset([poly], -offover, {
                minArea: 0,
                open: true,
              });
              if (pnew) {
  ...
  class OpPocket extends CamOp {
        ...
        for (let poly of polys) {
          ...
          for (let z of zs) {
            ...
            if (contour) {
              ...
              if (smooth) {
                shadow = POLY.setZ(
                  POLY.offset(POLY.offset(shadow, smooth), -smooth),
                  z
                );
              }
    ...
    conform(camLines, refine, engrave, progress) {
      ...
      for (let poly of hirez) {
        for (let point of poly.points) {
          point.z = engrave
            ? topo.zAtXY(point.x, point.y)
            : topo.toolAtXY(point.x, point.y);
        }
    ...
    prepare(ops, progress) {
      ...
      let { setTool, setSpindle, setTolerance, sliceOutput, getPrintPoint } =
        ops;
  ...
  class OpShadow extends CamOp {
    ...
    async slice(progress) {
      ...
      let terrain = await slicer.slice(tzindex, {
        each: (data) => {
          let shadow = trueShadow ? shadowAt(data.z, lsz) : [];
          tshadow = POLY.union(
            tshadow.slice().appendAll(data.tops).appendAll(shadow),
            0.01,
            true
          );
          ...
          if (false) {
            ...
            slice
              ...
              .addPolys(POLY.setZ(data.tops.clone(true), data.z), {
                thin: true,
              });
            // let p1 = [], p2 = [], cp = p1;
  ...
  CAM.shadowAt = function (widget, z, ztop) {
    ...
    for (let i = 0; i < faces.length; ) {
      ...
      if (a.z < z && b.z < z && c.z < z) {
        ...
        if (
          where.on.length === 0 &&
          (where.over.length === 2 || where.under.length === 2)
        ) {
