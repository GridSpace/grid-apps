...
const { poly2polyEmit, tip2tipEmit, segmentNormal, vertexNormal, arcToPath } = paths;
...
class OpHelical extends CamOp {
    ...
    async slice(progress){
        ...
        let { zBottom, zThru, thruHoles, color } = state;
        let { cylinders, forceStartAng, startAng, fromTop,offOver,offset, down, thru } = op
        ...
        for(let face of faces){
            ...
            let {zmin, zmax, diam, center,interior,faces} = CAM.cylinder_poly_find(widget, face)
            ...
            if(offOver){ //apply offset override
                if(offset == "auto"){
            ...
            }else{ //otherwise, calculate based on offset type
                if(offset == "auto"){
            ...
            let startPoint = center.clone().setZ(zmax).add(newPoint(Math.cos(startAngle) * radius, Math.sin(startAngle) * radius,0));
            ...
            for(;;){
                ...
                if(bottom > zBottom ){
                    ...
                    let toBottom = currentZ-zBottom,
                        ...
                        finalEnd= center.clone().setZ(zBottom).add(newPoint(Math.cos(finalAngle) * radius, Math.sin(finalAngle) * radius,0))
                    
                    //circle down to final point
                    poly.addPoints(arcToPath(
                        startPoint.setZ(currentZ),
                        finalEnd,numSegs*ofFull,
                        {
                            clockwise:!clockwise,
                            radius,
                            center
                    ...
                    poly.addPoints(arcToPath(finalEnd,finalEnd,numSegs,{clockwise,radius,center}))
