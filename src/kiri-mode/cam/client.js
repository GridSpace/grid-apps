...
gapp.register('kiri-mode.cam.client', [], (root, exports) => {
  ...
  CAM.init = function (kiri, api) {
    ...
    api.event.on(['selection.scale', 'selection.rotate'], () => {
      if (!isCamMode) return
    ...
    $('op-add-list').onclick = (ev) => {
      ...
      switch (ev.target.innerText.toLowerCase()) {
        case 'index':
          return func.opAddIndex()
        case 'laser on':
          return func.opAddLaserOn()
        case 'laser off':
          return func.opAddLaserOff()
        case 'gcode':
          return func.opAddGCode()
        case 'level':
          return func.opAddLevel()
        case 'rough':
          return func.opAddRough()
        case 'outline':
          return func.opAddOutline()
        case 'contour':
          let caxis = 'X'
          for (let op of current.process.ops) {
        ...
        case 'register':
          return func.opAddRegister('X', 2)
        case 'drill':
          return func.opAddDrill()
        case 'trace':
          return func.opAddTrace()
        case 'pocket':
          return func.opAddPocket()
        case 'flip':
    ...
    api.event.on(
      ...
      (func.opRender = () => {
        ...
        oplist.forEach((rec, i) => {
          ...
          let notable = rec.note
            ? rec.note.split(' ').filter((v) => v.charAt(0) === '#')
            : undefined
          if (clock) {
            clazz.push('clock')
            title = ` title="end of ops chain\ndrag/drop like an op\nops after this are disabled"`
          }
              ...
              for (let el of bounds) {
                ...
                if (tar.pageY >= top && tar.pageY <= bottom) {
                  ...
                  el.insertAdjacentElement(
                    tar.pageY < mid ? 'beforebegin' : 'afterend',
                    target
                  )
                }
              }
      ...
      CAM.traces((ids) => {
        ...
        kiri.api.widgets.for((widget) => {
          ...
          widget.traces.forEach((poly) => {
            ...
            layers
              .setLayer('trace', { line: 0xaaaa55, fat: 4, order: -10 }, false)
              .addPoly(poly)
            stack.addLayers(layers)
    ...
    func.selectHoles = async function (individual) {
      ...
      let meshesCached = widgets.every(
        (widget) => poppedRec.drills[widget.id] != undefined
      )
      if (individual && meshesCached) {
        ...
        await CAM.holes(individual ? 0 : diam, (centers) => {
          ...
          if (shadow) {
            alert = api.show.alert(
              'Some holes are shadowed by part and are not shown.'
            )
          }
          centers = centers ?? []
    ...
    function rotateTabs(widget, x, y, z) {
      ...
      tabs.forEach((rec) => {
        ...
        let tab = widget.tabs[id]
        let m4 = new THREE.Matrix4().makeRotationFromEuler(
          new THREE.Euler(x || 0, y || 0, z || 0)
        )
        // update position vector
        let vc = new THREE.Vector3(pos.x, pos.y, pos.z).applyMatrix4(m4)
        // update rotation quaternion
        let [rx, ry, rz, rw] = rot
        rec.rot = new THREE.Quaternion()
          .multiplyQuaternions(
    ...
    createPopOp('level', {
      ...
      spindle: UC.newInput(LANG.cc_spnd_s, {
        title: LANG.cc_spnd_l,
        convert: UC.toInt,
        show: hasSpindle,
      }),
      step: UC.newInput(LANG.cc_sovr_s, {
        title: LANG.cc_sovr_l,
        convert: UC.toFloat,
        bound: UC.bound(0.01, 1.0),
      }),
      rate: UC.newInput(LANG.cc_feed_s, {
        title: LANG.cc_feed_l,
        convert: UC.toInt,
        units: true,
      }),
      down: UC.newInput(LANG.cc_loff_s, {
        title: LANG.cc_loff_l,
        convert: UC.toFloat,
        units: true,
      }),
      over: UC.newInput(LANG.cc_lxyo_s, {
        title: LANG.cc_lxyo_l,
        convert: UC.toFloat,
        units: true,
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      stock: UC.newBoolean(LANG.cc_lsto_s, undefined, {
        title: LANG.cc_lsto_l,
      }),
    }
    ...
    createPopOp('rough', {
      ...
      plunge: UC.newInput(LANG.cc_plng_s, {
        title: LANG.cc_plng_l,
        convert: UC.toInt,
        units: true,
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      down: UC.newInput(LANG.cc_sdwn_s, {
        title: LANG.cc_sdwn_l,
        convert: UC.toFloat,
        units: true,
      }),
      step: UC.newInput(LANG.cc_sovr_s, {
      ...
      leave: UC.newInput(LANG.cr_lsto_s, {
        title: LANG.cr_lsto_l,
        convert: UC.toFloat,
        units: true,
      }),
      leavez: UC.newInput(LANG.cr_lstz_s, {
        title: LANG.cr_lstz_l,
        convert: UC.toFloat,
        bound: UC.bound(0, 10),
        units: true,
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      all: UC.newBoolean(LANG.cr_clst_s, undefined, {
        title: LANG.cr_clst_l,
        show: hasIndexing,
      }),
      voids: UC.newBoolean(LANG.cr_clrp_s, undefined, {
        title: LANG.cr_clrp_l,
      }),
      flats: UC.newBoolean(LANG.cr_clrf_s, undefined, {
        title: LANG.cr_clrf_l,
      }),
      inside: UC.newBoolean(LANG.cr_olin_s, undefined, {
        title: LANG.cr_olin_l,
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      exp: UC.newExpand('overrides'),
      ov_topz: UC.newInput(LANG.ou_ztop_s, {
        title: LANG.ou_ztop_l,
        convert: UC.toFloat,
        units: true,
      }),
      ov_botz: UC.newInput(LANG.ou_zbot_s, {
        title: LANG.ou_zbot_l,
        convert: UC.toFloat,
        units: true,
      }),
      ov_conv: UC.newBoolean(LANG.ou_conv_s, undefined, {
    ...
    createPopOp('outline', {
      ...
      step: UC.newInput(LANG.cc_sovr_s, {
        ...
        convert: UC.toFloat,
        bound: UC.bound(0.01, 1.0),
        show: () => popOp.outline.rec.wide,
      }),
      steps: UC.newInput(LANG.cc_sovc_s, {
        title: LANG.cc_sovc_l,
        convert: UC.toInt,
        bound: UC.bound(1, 5),
        show: () => popOp.outline.rec.wide,
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      top: UC.newBoolean(LANG.co_clrt_s, undefined, {
        title: LANG.co_clrt_l,
      }),
      inside: UC.newBoolean(LANG.co_olin_s, undefined, {
        title: LANG.co_olin_l,
        show: (op) => {
          return !op.inputs.outside.checked
        },
      }),
      outside: UC.newBoolean(LANG.co_olot_s, undefined, {
        title: LANG.co_olot_l,
        show: (op) => {
          return !op.inputs.inside.checked
        },
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      omitthru: UC.newBoolean(LANG.co_omit_s, undefined, {
        title: LANG.co_omit_l,
        xshow: (op) => {
          return op.inputs.outside.checked
        },
      }),
      omitvoid: UC.newBoolean(LANG.co_omvd_s, undefined, {
        title: LANG.co_omvd_l,
        xshow: (op) => {
          return op.inputs.outside.checked
        },
      }),
      wide: UC.newBoolean(LANG.co_wide_s, undefined, {
        title: LANG.co_wide_l,
        show: (op) => {
          return !op.inputs.inside.checked
        },
      }),
      dogbones: UC.newBoolean(LANG.co_dogb_s, undefined, {
        title: LANG.co_dogb_l,
        show: (op) => {
          return !op.inputs.wide.checked
        },
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      exp: UC.newExpand('overrides'),
    ...
    createPopOp('contour', {
      ...
      step: UC.newInput(LANG.cc_sovr_s, {
        title: LANG.cc_sovr_l,
        convert: UC.toFloat,
        bound: UC.bound(0.01, 10.0),
      }),
      leave: UC.newInput(LANG.cf_leav_s, {
        title: LANG.cf_leav_l,
        convert: UC.toFloat,
        bound: UC.bound(0, 100),
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      angle: UC.newInput(LANG.cf_angl_s, {
        title: LANG.cf_angl_l,
        convert: UC.toFloat,
        bound: UC.bound(45, 90),
        show: (op) => op.inputs.curves.checked,
      }),
      flatness: UC.newInput(LANG.ou_flat_s, {
        title: LANG.ou_flat_l,
        convert: UC.toFloat,
        bound: UC.bound(0, 1.0),
        units: false,
        round: 4,
      }),
      tolerance: UC.newInput(LANG.ou_toll_s, {
        title: LANG.ou_toll_l,
        convert: UC.toFloat,
        bound: UC.bound(0, 10.0),
        units: true,
        round: 4,
      }),
      reduction: UC.newInput(LANG.ou_redu_s, {
        title: LANG.ou_redu_l,
        convert: UC.toInt,
        bound: UC.bound(0, 10),
        units: false,
      }),
      ...
      sep: UC.newBlank({ class: 'pop-sep' }),
      curves: UC.newBoolean(LANG.cf_curv_s, undefined, {
        title: LANG.cf_curv_l,
      }),
      inside: UC.newBoolean(LANG.cf_olin_s, undefined, {
        title: LANG.cf_olin_l,
      }),
      bottom: UC.newBoolean(LANG.cf_botm_s, undefined, {
        title: LANG.cf_botm_l,
        show: (op, conf) => (conf ? conf.process.camZBottom : 0),
      }),
      filter: UC.newRow([UC.newButton(LANG.filter, contourFilter)], {
    ...
    createPopOp('lathe', {
      ...
      step: UC.newInput(LANG.cc_sovr_s, {
        title: LANG.cc_sovr_l,
        convert: UC.toFloat,
        bound: UC.bound(0.01, 100.0),
      }),
      angle: UC.newInput(LANG.cc_sang_s, {
        title: LANG.cc_sang_l,
        convert: UC.toFloat,
        bound: UC.bound(0.01, 180.0),
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      tolerance: UC.newInput(LANG.ou_toll_s, {
        title: LANG.ou_toll_l,
      ...
      sep: UC.newBlank({ class: 'pop-sep' }),
      linear: UC.newBoolean(LANG.ci_line_s, undefined, {
        title: LANG.ci_line_l,
      }),
    ...
    createPopOp('trace', {
      ...
      offset: UC.newSelect(
        LANG.cc_offs_s,
        {
          title: LANG.cc_offs_l,
          show: () => poppedRec.mode === 'follow',
        },
        'traceoff'
      ),
      sep: UC.newBlank({ class: 'pop-sep' }),
      ...
      step: UC.newInput(LANG.cc_sovr_s, {
        ...
        convert: UC.toFloat,
        bound: UC.bound(0.01, 1.0),
        show: (op) => popOp.trace.rec.mode === 'clear',
      }),
      down: UC.newInput(LANG.cc_sdwn_s, {
        title: LANG.cc_sdwn_l,
        convert: UC.toFloat,
      ...
      thru: UC.newInput(LANG.cc_thru_s, {
        title: LANG.cc_thru_l,
        convert: UC.toFloat,
        units: true,
      }),
      offover: UC.newInput(LANG.cc_offd_s, {
        title: LANG.cc_offd_l,
        convert: UC.toFloat,
        units: true,
        show: () => poppedRec.offset !== 'none' || poppedRec.mode === 'clear',
      }),
      sep: UC.newBlank({ class: 'pop-sep', modes: MCAM, xshow: zDogSep }),
      merge: UC.newBoolean(LANG.co_merg_s, undefined, {
        title: LANG.co_merg_l,
        show: () => !popOp.trace.rec.down,
      }),
      dogbone: UC.newBoolean(LANG.co_dogb_s, undefined, {
        title: LANG.co_dogb_l,
        show: canDogBones,
      }),
      revbone: UC.newBoolean(LANG.co_dogr_s, undefined, {
        title: LANG.co_dogr_l,
        show: canDogBonesRev,
      }),
      exp: UC.newExpand('overrides'),
      sep: UC.newBlank({ class: 'pop-sep' }),
      ...
      exp_end: UC.endExpand(),
      sep: UC.newBlank({ class: 'pop-sep' }),
      menu: UC.newRow([UC.newButton('select', func.traceAdd)], {
        class: 'ext-buttons f-row',
      }),
    }
    ...
    createPopOp('pocket', {
      ...
      down: UC.newInput(LANG.cc_sdwn_s, {
        ...
        convert: UC.toFloat,
        units: true,
        show: () => !poppedRec.contour,
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      expand: UC.newInput(LANG.cp_xpnd_s, {
        title: LANG.cp_xpnd_l,
        convert: UC.toFloat,
        units: true,
        xshow: () => !poppedRec.contour,
      }),
      refine: UC.newInput(LANG.cp_refi_s, {
        title: LANG.cp_refi_l,
        convert: UC.toInt,
        show: () => poppedRec.contour,
      }),
      smooth: UC.newInput(LANG.cp_smoo_s, {
        title: LANG.cp_smoo_l,
        convert: UC.toInt,
      }),
      tolerance: UC.newInput(LANG.ou_toll_s, {
        title: LANG.ou_toll_l,
      ...
      follow: UC.newInput(LANG.cp_foll_s, {
        title: LANG.cp_foll_l,
        convert: UC.toFloat,
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      contour: UC.newBoolean(LANG.cp_cont_s, undefined, {
        title: LANG.cp_cont_s,
      }),
      engrave: UC.newBoolean(LANG.cp_engr_s, undefined, {
        title: LANG.cp_engr_l,
        show: () => poppedRec.contour,
      }),
      outline: UC.newBoolean(LANG.cp_outl_s, undefined, {
        title: LANG.cp_outl_l,
      }),
      exp: UC.newExpand('overrides'),
      sep: UC.newBlank({ class: 'pop-sep' }),
      ov_topz: UC.newInput(LANG.ou_ztop_s, {
      ...
      exp_end: UC.endExpand(),
      sep: UC.newBlank({ class: 'pop-sep' }),
      menu: UC.newRow([UC.newButton('select', func.surfaceAdd)], {
        class: 'ext-buttons f-row',
      }),
    }
    ...
    drillOp.inputs = {
      ...
      rate: UC.newInput(LANG.cc_plng_s, {
        title: LANG.cc_plng_l,
        convert: UC.toInt,
        units: true,
      }),
      ...
      dwell: UC.newInput(LANG.cd_dwll_s, {
        title: LANG.cd_dwll_l,
        convert: UC.toFloat,
      }),
      lift: UC.newInput(LANG.cd_lift_s, {
        title: LANG.cd_lift_l,
        convert: UC.toFloat,
        units: true,
        show: () => !poppedRec.mark,
      }),
      mark: UC.newBoolean(LANG.cd_mark_s, undefined, {
        title: LANG.cd_mark_l,
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      thru: UC.newInput(LANG.cd_dtru_s, {
        title: LANG.cd_dtru_l,
        convert: UC.toFloat,
        units: true,
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      actions: UC.newRow(
        [
          UC.newButton(LANG.select, () => func.selectHoles(true), {
            title: LANG.cd_seli_l,
          }),
          UC.newButton(LANG.cd_sela_s, () => func.selectHoles(false), {
            title: LANG.cd_sela_l,
          }),
        ],
    ...
    createPopOp('register', {
      ...
      feed: UC.newInput(LANG.cc_feed_s, {
        title: LANG.cc_feed_l,
        convert: UC.toInt,
        units: true,
        show: () => poppedRec.axis === '-',
      ...
      sep: UC.newBlank({ class: 'pop-sep' }),
      thru: UC.newInput(LANG.cd_thru_s, {
        title: LANG.cd_thru_l,
        convert: UC.toFloat,
        units: true,
      }),
    }
    ...
    createPopOp('flip', {
      ...
      invert: UC.newBoolean(LANG.cf_nvrt_s, undefined, {
        title: LANG.cf_nvrt_l,
        show: zBottom,
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      action: UC.newRow([UC.newButton(LANG.cf_menu, func.opFlip)], {
        class: 'ext-buttons f-row',
      }),
    }
    ...
    createPopOp('gcode', {
      ...
      action: UC.newRow([UC.newButton(LANG.edit, gcodeEditor())], {
        class: 'ext-buttons f-row',
      }),
    }
    ...
    createPopOp('index', {
      ...
      degrees: UC.newInput(LANG.ci_degr_s, {
        title: LANG.ci_degr_l,
        convert: UC.toFloat,
        bound: UC.bound(-360, 360.0),
      }),
      absolute: UC.newBoolean(LANG.ci_abso_s, undefined, {
        title: LANG.ci_abso_l,
      }),
      select: UC.newRow([UC.newButton(LANG.ci_face_s, angleTowardZUp)], {
        class: 'ext-buttons f-row',
      }),
    }
    ...
    createPopOp('laser on', {
      ...
      enable: UC.newRow([UC.newButton(LANG.enable, editEnable)], {
        class: 'ext-buttons f-row',
      }),
      on: UC.newRow([UC.newButton(LANG.on, editOn)], {
        class: 'ext-buttons f-row',
      }),
      off: UC.newRow([UC.newButton(LANG.off, editOff)], {
        class: 'ext-buttons f-row',
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      power: UC.newInput(LANG.cl_powr_s, {
        title: LANG.cl_powr_l,
        convert: UC.toFloat,
        bound: UC.bound(0, 1.0),
        show: () => !poppedRec.adapt,
      }),
      maxp: UC.newInput(LANG.cl_maxp_s, {
        title: LANG.cl_maxp_l,
        convert: UC.toFloat,
        bound: UC.bound(0, 1.0),
        show: () => poppedRec.adapt,
      }),
      minp: UC.newInput(LANG.cl_minp_s, {
        title: LANG.cl_minp_l,
        convert: UC.toFloat,
        bound: UC.bound(0, 1.0),
        show: () => poppedRec.adapt,
      }),
      maxz: UC.newInput(LANG.cl_maxz_s, {
        title: LANG.cl_maxz_l,
        convert: UC.toFloat,
        show: () => poppedRec.adapt,
      }),
      minz: UC.newInput(LANG.cl_minz_s, {
        title: LANG.cl_minz_l,
        convert: UC.toFloat,
        show: () => poppedRec.adapt,
      }),
      flatz: UC.newInput(LANG.cl_flaz_s, {
        title: LANG.cl_flaz_l,
        convert: UC.toFloat,
        show: () => poppedRec.flat,
      }),
      sep: UC.newBlank({ class: 'pop-sep' }),
      adapt: UC.newBoolean(LANG.cl_adap_s, undefined, {
        title: LANG.cl_adap_l,
      }),
      adaptrp: UC.newBoolean(LANG.cl_adrp_s, undefined, {
        title: LANG.cl_adrp_l,
        show: () => poppedRec.adapt,
      }),
      flat: UC.newBoolean(LANG.cl_flat_s, undefined, {
        title: LANG.cl_flat_l,
      }),
    }
    ...
    createPopOp('laser off', {
      ...
      disable: UC.newRow([UC.newButton(LANG.disable, editDisable)], {
        class: 'ext-buttons f-row',
      }),
    }
    ...
    let op = (popOp[type] = {
      ...
      bind: (ev) => {
        ...
        if (opType != 'drill' && tool.type == 'drill') {
          alerts.show(
            `Warning: Drills should not be used for ${opType} operations.`
          )
        } else if (opType == 'drill' && tool.type != 'drill') {
          alerts.show(
            `Warning: Only drills should be used for drilling operations.`
          )
        }
      ...
      addNote: () => {
        if (!op.note && type !== 'flip') {
          ...
          div.innerHTML = h.build(h.div([h.label({ id: noteid })]))
        ...
        if (op.note) {
          ...
          if (div)
            div.onclick = () => {
              API.uc
                .prompt('Edit Note for Operation', poppedRec.note || '')
                .then((note) => {
  ...
  function createTabBox(iw, ic, n) {
    ...
    const { camTabsWidth, camTabsHeight, camTabsDepth, camTabsMidline } =
      process
    ...
    const zp =
      (camZBottom || isIndexed ? camZBottom : sz - track.box.d - zto) +
      (camTabsMidline ? 0 : camTabsHeight / 2)
    ...
    const tab = addbox(pos, boxColor(), 'tabb', dim, {
      rotate: rot,
      opacity: boxOpacity(),
    })
    return {
  ...
  function addWidgetTab(widget, rec) {
    ...
    if (!tabs[id]) {
      pos.box = addbox(pos, boxColor(), id, dim, {
        group: widget.mesh,
        rotate: rot,
        opacity: boxOpacity(),
      })
  ...
  function addbox() {
    return FDM.addbox(...arguments)
  }

  function delbox() {
    return FDM.delbox(...arguments)
  }
  ...
  function updateStock() {
    ...
    if (x && y && z) {
      ...
      if (!camStock) {
        ...
        ligeo.setAttribute(
          'position',
          new THREE.BufferAttribute(lidat.toFloat32(), 3)
        )
      ...
      camStock.lines.material.color = new THREE.Color(
        API.space.is_dark() ? 0x555555 : 0xaaaaaa
      )
    } else if (camStock) {
      SPACE.world.remove(camStock)
      camStock = null
