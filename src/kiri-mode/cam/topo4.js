...
gapp.register('kiri-mode.cam.topo', [], (root, exports) => {
  ...
  class Topo4 {
    ...
    async generate(opt = {}) {
      ...
      let axis = op.axis.toLowerCase(),
        ...
        resolution = tolerance
          ? tolerance
          : 1 / Math.sqrt(density / (span.x * span.y)),
      ...
      for (let slice of slices) {
        const recs = new kiri.topo_slicer(slice.index)
          ...
          .map((rec) => {
            ...
            const points = codec.encodePointArray(
              rec.lines.map((l) => [l.p1, l.p2]).flat()
            );
            const shared = new Float32Array(
              new SharedArrayBuffer(points.length * 4)
            );
            shared.set(points);
            slice.shared = shared;
    ...
    async sliceMinions(onupdate) {
      ...
      const output = codec
        .decode(await Promise.all(promises))
        .map((rec) => rec.recs)
        .flat()
        .map((rec) =>
          newSlice(rec.z).addTops(rec.polys).setFields({ shared: rec.shared })
        )
        .sort((a, b) => a.z - b.z);
    ...
    async latheWorker(onupdate) {
      ...
      for (let rec of recs) {
        ...
        [...heights].group(3).forEach((a, i) => {
          ...
          paths[i].camLines[0].push(
            newPoint(a[0], a[1], a[2] + leave).setA(degrees + i * -360)
          );
        });
      }
      ...
      for (let slice of paths) {
        ...
        slice
          ...
          .addPoly(
            poly.clone().applyRotations().move({ z: -zoff, x: 0, y: 0 })
          );
      }
      ...
      if (linear) {
        recs.forEach((rec, i) => {
          ...
          [...heights].group(3).forEach((a) => {
            paths[i].camLines[0].push(
              newPoint(a[0], a[1], a[2] + leave).setA(degrees)
            );
          });
          if (i % 2 === 1) {
