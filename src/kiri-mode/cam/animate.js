...
gapp.register('kiri-mode.cam.animate', [], (root, exports) => {
  ...
  kiri.load(() => {
    ...
    function createGrid(stepsX, stepsY, size, step, stock) {
      ...
      for (let x = 0, ai = 0; x < stepsX; x++) {
        for (let y = 0; y < stepsY; y++) {
          ...
          pos[ai++] =
            stock && (x * y === 0 || x === ex || y === ey) ? 0 : size.z
          if (asPoints) {
            continue
          }
          if (asLines) {
            if (y > 0) ind.appendAll([stepsY * x + (y - 1), stepsY * x + y])
            if (x > 0) ind.appendAll([stepsY * (x - 1) + y, stepsY * x + y])
          } else {
            if (x > 0 && y > 0) {
              ...
              let v2 = stepsY * (x - 0) + y
              let v3 = stepsY * (x - 1) + y
              ind.appendAll([v0, v1, v2, v0, v2, v3])
            }
          }
        }
      }
    ...
    function updateTool(toolobj, send) {
      ...
      const prof = tool.profile
      const { size, pix } = tool.profileDim
      const { pos, ind, sab } = createGrid(
        pix,
        pix,
        { x: size, y: size, z: flen + slen },
        rez
      )
      const mid = Math.floor(pix / 2)
