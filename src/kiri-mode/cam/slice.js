  ...
  CAM.slice = async function (settings, widget, onupdate, ondone) {
    ...
    for (let op of opList) {
      ...
      if (op.op.type === 'index') {
        ...
        shadows = {}
        await new CAM.OPS.shadow(state, {
          type: 'shadow',
          silent: true,
        }).slice((progress) => {
    ...
    for (let slice of slices) {
      for (let top of slice.tops) {
        ...
        for (let poly of inner) {
          ...
          center.selected =
            !individual &&
            poly.circularity() >= 0.985 &&
            Math.abs(center.area - area) <= areaDelta
          ...
          for (let [i, drill] of drills.entries()) {
            ...
            if (dist <= centerDiff && drill.selected == center.selected) {
              ...
              if (center.z > drill.z) {
                //if current is higher than old
                drills[i] = center //replace with top point
                drills[i].depth = center.z - drill.z
              } else if (center.z <= drill.z - drill.depth) {
                // if current is lower or same as old
