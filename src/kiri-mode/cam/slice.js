...
gapp.register('kiri-mode.cam.slice', (root, exports) => {
  ...
  CAM.slice = async function (settings, widget, onupdate, ondone) {
    ...
    if (stock.x && stock.y && stock.z) {
      if (stock.x + 0.00001 < bounds.max.x - bounds.min.x) {
        return error(
          'stock X too small for part. resize stock or use offset stock'
        )
      }

      if (stock.y + 0.00001 < bounds.max.y - bounds.min.y) {
        return error(
          'stock Y too small for part. resize stock or use offset stock'
        )
      }

      if (stock.z + 0.00001 < bounds.max.z - bounds.min.z) {
        return error(
          'stock Z too small for part. resize stock or use offset stock'
        )
      }
    }
    ...
    if (zMin >= bounds.max.z) {
      return error(
        `invalid z bottom ${(zMin / units).round(3)} >= bounds z max ${(zMax / units).round(3)}`
      )
    }
    ...
    let opSum = 0
    let opTot = opList.length
      ? opList.map((op) => op.weight()).reduce((a, v) => a + v)
      : 0
    ...
    for (let op of opList) {
      ...
      if (op.op.type === 'index') {
        ...
        shadows = {}
        await new CAM.OPS.shadow(state, { type: 'shadow', silent: true }).slice(
          (progress) => {
    ...
    let shadowOff =
      maxToolDiam < 0
        ? allpoly
        : POLY.offset(allpoly, [minToolDiam / 2, maxToolDiam / 2], {
            count: 2,
            flat: true,
            minArea: 0,
          })
    ...
    let indices = [
      ...new Set(
        Object.keys(zees)
          ...
          .appendAll(
            Object.entries(single ? slicer.zLine : {})
              .map((ze) => {
                let [zk, zv] = ze
  ...
  CAM.holes = async function (settings, widget, diam) {
    ...
    let zFlats = Object.keys(slicer.zFlat)
      .map(Number)
      .map((z) => [z])
      .flat()
    ...
    let indices = [
      ...new Set(
        zees.map((kv) => parseFloat(kv).round(5)).filter((z) => z !== null)
      ),
    ]
    ...
    for (let c of circles) {
      let overlapping = c.overlapping.sort((a, b) => b.z - a.z)
      ...
      while (overlapping.length) {
        ...
        if (areaDelta < aveArea * 0.05) {
          // if area delta less than 5% of average area
          //keep top circle selected
          last.depth = last.z - circ.z
        } else {
          // if not the same area
          //push and move on
          drills.push(last)
          last = circ
        }
  ...
  function cutTabs(tabs, offset, z, inter) {
    tabs = tabs
      .filter((tab) => z < tab.pos.z + tab.dim.z / 2)
      .map((tab) => tab.off)
      .flat()
  ...
  function cutPolys(polys, offset, z, inter) {
    let noff = []
    offset.forEach((op) =>
      noff.appendAll(op.cut(POLY.union(polys, 0, true), inter))
    )
