...
gapp.register('kiri-mode.sla.x_photons', [], (root, exports) => {
  ...
  function generatePhoton(print, conf, progress) {
    ...
    for (let sc = 0; sc < subcount; sc++)
      for (let l = 0; l < layers.length; l++) {
        ...
        filedat.writeF32(
          l < process.slaBaseLayers ? process.slaBaseOn : process.slaLayerOn,
          true
        );
        filedat.writeF32(
          l < process.slaBaseLayers ? process.slaBaseOff : process.slaLayerOff,
          true
        );
  ...
  function writePhotonImage(preview, writer) {
    let data = new Uint8Array(preview.data),
      len = data.byteLength;
