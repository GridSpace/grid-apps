...
gapp.register('kiri-mode.fdm.slice', [], (root, exports) => {
  ...
  FDM.slice = function (settings, widget, onupdate, ondone) {
    let render = settings.render !== false,
      ...
      extruder = Math.min(
        maxtruder,
        parseInt(isSynth ? process.sliceSupportNozzle : metadata.extruder || 0)
      ),
      sliceHeight = process.sliceHeight,
      sliceHeightBase =
        (isBelt ? sliceHeight : process.firstSliceHeight) || sliceHeight,
      lineWidth =
        process.sliceLineWidth || device.extruders[extruder].extNozzle,
    ...
    const sliceMinHeight =
      process.sliceAdaptive && process.sliceMinHeight > 0
        ? Math.min(process.sliceMinHeight, sliceHeight)
        : 0
      ...
      .slice(points, {
        ...
        zGen(zopt) {
          ...
          if (hm) {
            ...
            let zOrdered = Object.values(zopt.zIndexes).map((v) =>
              parseFloat(v)
            )
        ...
        slicer(z, points, opts) {
          ...
          return (isConcurrent ? kiri.minions.sliceZ : base.sliceZ)(
            z,
            points,
            opts
          )
    ...
    async function onSliceDone(slices) {
      ...
      if (
        !isBelt &&
        (isSynth || (!isSynth && supportDensity && process.sliceSupportEnable))
      ) {
      ...
      forSlices(
        ...
        (slice) => {
          ...
          let spaceMult =
            slice.index === 0 ? process.firstLayerLineMult || 1 : 1
          ...
          let isSolid =
            (isBottom || ((isTop || isDense) && !vaseMode)) && !isSynth
      ...
      if (isBelt && !isSynth) {
        ...
        for (let slice of slices) {
          ...
          slice.belt = {
            miny,
            touch: miny.round(3) < sliceHeightBase,
          }
        }
        ...
        let anchorlen =
          (process.beltAnchor || process.firstLayerBeltLead) * cosf
        ...
        for (let slice of slices) {
          if (slice._clone_sparse) {
            ...
            for (let i = 0; i < tops.length; i++) {
              tops[i].fill_sparse = down[i].fill_sparse.map((p) =>
                p.cloneZ(slice.z)
              )
            }
          }
        ...
        forSlices(
          ...
          (slice) => {
            ...
            doSupportFill({
              ...
              lineWidth,
              density,
              minArea: process.sliceSupportArea,
              isBelt,
              angle: process.sliceSupportFill,
  ...
  function doRender(slice, isSynth, params, opt = {}) {
    ...
    if (slice.tops)
      // missing for supports
      slice.tops.forEach((top) => {
        if (isThin) output.setLayer('part', Color.part).addPolys([top.poly])

        output
          .setLayer(
            isSynth ? 'support' : 'shells',
            isSynth ? Color.support : Color.shell
          )
        ...
        output
          ...
          .addLines(
            top.fill_lines || [],
            vopt({
              offset: offset * solidWidth,
              height,
              z: slice.z,
            })
          )

        if (!(slice.belt && slice.belt.anchor))
          output
            .setLayer('sparse fill', Color.infill)
            .addPolys(
              top.fill_sparse || [],
              vopt({ offset, height, outline: true, trace: true })
            )
    ...
    if (isThin && devel) {
      if (slice.solids && slice.solids.length)
        output
    ...
    if (slice.xray) {
      ...
      if (slice.lines) {
        slice.lines.forEach((line, i) => {
          ...
          slice
            .output()
            .setLayer(`l${group}`, color[group])
            .addLine(line.p1, line.p2)
        })
      }
      if (slice.groups)
  ...
  function doSparseLayerFill(slice, options = {}) {
    ...
    let tops = slice.tops,
      ...
      target = {
        ...
        repeat: function () {
          return process.sliceFillRepeat
        },
        // output functions
        emit: function (x, y) {
          if (isNaN(x)) {
    ...
    if (skippable && slice.fingerprintSame(down)) {
      ...
      if (
        down._fill_finger &&
        POLY.fingerprintCompare(slice._fill_finger, down._fill_finger)
      ) {
        for (let i = 0; i < tops.length; i++) {
    ...
    if (
      clip.Execute(ctyp.ctIntersection, ctre, cfil.pftNonZero, cfil.pftEvenOdd)
    ) {
  ...
  function doSolidsFill(slice, spacing, angle, minArea, fillQ) {
    ...
    let unioned = POLY.union(solids, undefined, true, {
        wasm: true,
      }).flat(),
    ...
    let tops_area = tops.length
      ? tops.map((top) => top.poly.areaDeep()).reduce((a, i) => a + i)
      : 0
    for (let solid of solids) {
      for (let top of tops) {
        let stop = []
  ...
  function doFillArea(fillQ, polys, angle, spacing, output, minLen, maxLen) {
    if (fillQ) {
      fillQ.push(
        kiri.minions.fill(polys, angle, spacing, output, minLen, maxLen)
      )
    } else {
  ...
  async function doSupport(slice, proc, shadow, opt = {}) {
    ...
    if (opt.exp && down_tops) {
      ...
      if (points > 200) {
        ...
        down_traces = down_traces.map((p) =>
          p.clean(true, undefined, config.clipper / 10)
        )
      }
    }
    ...
    function checkPointSupport(point) {
      ...
      if (!supported)
        down_traces.forEach(function (trace) {
    ...
    if (slice.down)
      (function () {
        ...
        traces.forEach(function (trace) {
          trace.forEachSegment(function (p1, p2) {
            checkLineSupport(p1, p2, true)
          })
        })

        // add offset solids to supports (or fill depending)
        fill.forEachPair(function (p1, p2) {
          checkLineSupport(p1, p2, false)
        })
        ...
        points.forEach(function (point) {
          pillars.push(
            base.newPolygon().centerRectangle(point, size / 2, size / 2)
          )
        })
        ...
        pillars = POLY.union(pillars, null, true, {
          wasm: false,
    ...
    if (SDBG && down_traces)
      slice
        ...
        .addPolys(
          cks.map((p) => base.newPolygon().centerRectangle(p, 0.25, 0.25))
        )
        .setLayer('pip', { line: 0xdd4422, check: 0xdd4422 })
        .addPolys(
          pip.map((p) => base.newPolygon().centerRectangle(p, 0.4, 0.4))
        )
        .setLayer('pcl', { line: 0xcc3311, check: 0xcc3311 })
        .addPolys(
          pcl.map((p) => base.newPolygon().centerRectangle(p, 0.3, 0.3))
        )
        .setLayer('pts', { line: 0xdd33dd, check: 0xdd33dd })
        .addPolys(
          points.map((p) => base.newPolygon().centerRectangle(p, 0.8, 0.8))
        )
  ...
  function doSupportFill(args) {
    ...
    // union supports
    supports = POLY.setZ(
      POLY.union(supports, undefined, true, { wasm: false }),
      slice.z
    )
  ...
  function fillSupportPolys(args) {
    const { promises, polys, lineWidth, density, z, isBelt, angle, outline } =
      args
    ...
    polys.forEach(function (poly) {
      ...
      let auto =
        isBelt || poly.bounds.width() / poly.bounds.height() > 1 ? 1090 : 1000
      // inset support poly for fill lines 33% of nozzle width
      let inset = POLY.offset([poly], -lineWidth / 3, {
        flat: true,
        z,
        wasm: true,
      })
  ...
  FDM.supports = function (settings, widget) {
    ...
    let platform = new THREE.Mesh(new THREE.PlaneGeometry(10000, 10000, 1), mat)
    const now = Date.now()
    // test point
    function tp(point) {
      if (point.added) {
