...
gapp.register('kiri-mode.fdm.prepare', [], (root, exports) => {
  ...
  FDM.prepare = async function (widgets, settings, update) {
    ...
    if (!isBelt && (process.outputBrimCount || useRaft)) {
      ...
      if (useRaft) {
        let offset = newPoint(0, 0, 0),
          ...
          rafts = process.outputBrimCount
            ? POLY.expand(brims, nozzle * 4, 0, null, 1)
            : brims
        ...
        // emit raft layers
        raft(nozzle * 1.5, angle + 0, nozzle * 6.0, rate1, 2.5, true)
        raft(nozzle * 0.5, angle + 90, nozzle * 2.0, rate2, 1.5, true)
        raft(nozzle * 0.5, angle + 0, nozzle * 1.0, rate2, 1.0)
      ...
      if (process.outputBrimCount) {
        ...
        // output brim points
        let brimStart =
          offset < nozzle * 2 ? newPoint(-bedWidth, -bedDepth, 0) : printPoint
        printPoint = poly2polyEmit(
          polys,
          brimStart,
          (poly, index, count, startPoint) => {
    ...
    function mkrec(i, angle = 45, thin = 6) {
      let exi = device.extruders[i],
        noz = exi.extNozzle,
        pos = {
          x: blokpos.x + walkpos.x * i,
          y: blokpos.y + walkpos.y * i,
          z: 0,
        },
    ...
    function purge(nozzle, track, layer, start, z, using, offset) {
      ...
      let first = isBelt
        ? !purgedFirst && layer.slice.index >= 0
        : layer.slice.index === 0
      ...
      if (rec) {
        ...
        if (isBelt && z < 0) {
          ...
          rate =
            (process.outputFeedrate - process.firstLayerRate) * scale +
        ...
        if (fill && fill.length) {
          ...
          start = print.polyPrintPath(fill, start, layer, {
            ...
            open: true,
            onfirst: (point) => {
              point.purgeOn = purgeOn ? pause : undefined
            },
          })
        }
        layer.last().retract = true
      ...
      for (let layer of output) {
        ...
        for (let rec of layer) {
          ...
          if (
            ...
            belty <= thresh &&
            lastout &&
            Math.abs(lastout.belty - belty) < 0.01
          ) {
        ...
        let trigmet =
          ...
          (firstLayerBrimTrig && mins <= firstLayerBrimTrig)
  ...
  function slicePrintPath(print, slice, startPoint, offset, output, opt = {}) {
    ...
    let preout = [],
      ...
      shellMult =
        opt.mult ||
        process.outputShellMult ||
        (process.ctSliceHeight >= 0 ? 1 : 0),
      shellOrder =
        { 'out-in': -1, 'in-out': 1, alternate: -2 }[process.sliceShellOrder] ||
        -1,
      ...
      printSpeed =
        opt.speed || (firstLayer ? firstShellSpeed : process.outputFeedrate),
      fillSpeed =
        opt.speed ||
        opt.fillSpeed ||
        (firstLayer
          ? firstFillSpeed || firstShellSpeed
          : process.outputFeedrate),
      infillSpeed =
        process.sliceFillRate || opt.infillSpeed || fillSpeed || printSpeed,
      ...
      switchTop =
        print.lastPoly && print.lastPoly.perimeter() < process.outputShortPoly,
      z = slice.z,
      lastPoly
    ...
    function routeAround(p1, p2) {
      const dbug = false
      if (dbug === slice.index)
        console.log(slice.index, { p1, p2, d: p1.distTo2D(p2) })
      ...
      if (ints.length && ints.length % 2 !== 0) {
        if (dbug === slice.index)
          console.log(slice.index, { odd_intersects: ints })
        return false
      }
      ...
      for (let i = 0; i < ints.length; i += 2) {
        ...
        if (i1.poly !== i2.poly) {
          if (dbug === slice.index)
            console.log(slice.index, { int_diff_poly: ints, i })
          return false
        }
        ...
        if (i1.ip.distTo2D(i2.ip) < retractDist) {
          if (dbug === slice.index)
            console.log(slice.index, {
              int_dist_too_small: i1.ip.distTo2D(i2.ip),
              retractDist,
            })
          ints[i] = undefined
          ints[i + 1] = undefined
      ...
      if (ints.length > 2) {
        if (dbug === slice.index)
          console.log(slice.index, { complex_route: ints.length })
        return false
      }
    ...
    function outputThin(lines) {
      ...
      let points = lines.group(2).map((grp) => {
        let [p1, p2] = grp
        return newPoint((p1.x + p2.x) / 2, (p1.y + p2.y) / 2, (p1.z + p2.z) / 2)
      })
      ...
      if (
        startPoint &&
        startPoint.distTo2D(first) > thinWall &&
        intersectsTop(startPoint, first)
      ) {
        retract()
      }
      ...
      while (lines && marked < lines.length) {
        ...
        if (near)
          for (let i = 0; i < lines.length; i += 2) {
            ...
            let idiff = startPoint.index
              ? Math.abs(startPoint.index - t1.index)
              : 1
        ...
        if (dist <= thinDist && len <= thinDist) {
          ...
          if (
            !fast &&
            dist > retractDist &&
            (zhop || intersectsTop(startPoint, p1))
          ) {
            retract()
          }
          ...
          if (!fast && antiBacklash && dist > retractDist) {
            print.addOutput(
              preout,
              p1.add({ x: antiBacklash, y: -antiBacklash, z: 0 }),
              0,
              moveSpeed,
              extruder
            )
      ...
      function (next) {
        if (next instanceof Polygon) {
          ...
          switch (process.sliceLayerStart) {
            ...
            case 'position':
              startPoint = newPoint(
                -bedOffset.x + process.sliceLayerStartX,
                -bedOffset.y + process.sliceLayerStartY,
                startPoint.z
              )
              break
          }
          ...
          if (shellOrder > 0)
            outputTraces(inner, { sort: shellOrder, shell: true })
