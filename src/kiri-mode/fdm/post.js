...
gapp.register('kiri-mode.fdm.post', [], (root, exports) => {
  ...
  base.slicePost.FDM = function (data, options) {
    ...
    if (isSynth) {
      ...
      if (process.sliceSupportGrow > 0) {
        ...
        data.tops = POLY.expand(
          data.tops,
          process.sliceSupportGrow,
          data.z,
          []
        );
      }
  ...
  function offset_default(params) {
    let { z, top, count, top_poly, offset1, offsetN, wasm, last, gaps } =
      params;
  ...
  function thin_type_2(params) {
    let { z, top, count, top_poly, offset1, offsetN, last, gaps, wasm } =
      params;
