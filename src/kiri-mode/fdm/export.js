...
gapp.register('kiri-mode.fdm.export', [], (root, exports) => {
  ...
  FDM.export = function (print, online, ondone, ondebug) {
    ...
    for (let line of gcpre) {
      ...
      else if (line.indexOf('T1') === 0) t1 = true;
      else if (line.indexOf('M82') === 0) {
        extrudeAbs = true;
        extrudeSet = true;
      } else if (line.indexOf('M83') === 0) {
        extrudeAbs = false;
        extrudeSet = true;
      } else if (line.indexOf('G90') === 0 && !extrudeSet) extrudeAbs = true;
      else if (line.indexOf('G91') === 0 && !extrudeSet) extrudeAbs = false;
    ...
    while (layer < layers.length) {
      ...
      emitPerMM = extrudePerMM(
        ...
        path.layer === 0
          ? process.firstSliceHeight || process.sliceHeight
          : path.height
      );
      ...
      if (loops) {
        for (let loop of loops) {
          if (layerno === loop.start && !loop.started) {
            ...
            if (extrudeAbs) {
              append(
                `G92 Z${lout.z.round(decimals)} E${outputLength.round(decimals)}`
              );
            } else {
      ...
      if (gcodeLayer && gcodeLayer.length) {
        ...
        append(
          `;; --- layer ${layer} (${subst.height.toFixed(3)} @ ${subst.z.round(3)}) ---`
        );
      }
      ...
      for (pidx = 0; pidx < path.length; pidx++) {
            ...
            if (arcQ.length > 2) {
              ...
              if (cc) {
                ...
                if (arcQ.length === 3) {
                  ...
                  arcQ.xSum = arcQ.center.reduce(function (t, v) {
                    return t + v.x;
                  }, 0);
                  arcQ.ySum = arcQ.center.reduce(function (t, v) {
                    return t + v.y;
                  }, 0);
                  arcQ.rSum = arcQ.center.reduce(function (t, v) {
                    return t + v.r;
                  }, 0);
                ...
                if (
                  ...
                  depm ||
                  desp ||
                  (dc * arcQ.center.length) / arcQ.rSum > arcDist ||
                  dist > cc.r ||
                  cc.r > arcMax ||
                  radFault ||
                  !arcValid()
                  ...
                  if (arcQ.length === 4) {
                    ...
                    if (!tc) {
                      ...
                      if (Math.abs(angle) > (Math.PI * 2) / arcRes) {
                        // enforce arcRes on initial angle
                        emitQrec(arcQ.shift());
                      }
                    }
                  } else {
        ...
        // }
        distance += dist;
        subst.progress = progress = Math.round(
    ...
    function drainQ() {
      ...
      if (arcQ.length > 4) {
        ...
        let vec2 = new THREE.Vector2(
          arcQ.center[0].x - arcQ[0].x,
          arcQ.center[0].y - arcQ[0].y
        );
        ...
        append(
          `${pre}${add} ; merged=${cl - 1} len=${dist.toFixed(decimals)} cp=${cc.x.round(2)},${cc.y.round(2)}`
        );
        pos.x = to.x;
        pos.y = to.y;
        pos.z = to.z;
    ...
    function arcValid() {
      ...
      for (let point of arcQ) {
        let rad = Math.sqrt(
          Math.pow(point.x - ac.x, 2) + Math.pow(point.y - ac.y, 2)
        );
