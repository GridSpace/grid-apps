...
gapp.main('main.mesh', [], (root) => {
  ...
  async function restore_space() {
    ...
    // let mcache = {};
    await db_admin.get('camera').then((saved) => {
      if (saved) {
    ...
    space.event.addHandlers(self, [
      ...
      (evt) => {
        ...
        switch (code) {
          ...
          case 'KeyG':
            return shiftKey
              ? api.mode.is([api.modes.sketch])
                ? api.sketch.arrange.group()
                : api.tool.regroup()
              : api.grid()
          case 'KeyL':
      ...
      (evt) => {
        ...
        let rot,
          floor = api.prefs.map.space.floor !== false
        switch (code) {
          case 'KeyA':
            if (metaKey || ctrlKey) {
          ...
          case 'Delete':
            ...
            if (
              [api.modes.object, api.modes.tool, api.modes.sketch].contains(
                mode
              )
            ) {
              selection.delete()
      ...
      if (event?.target?.nodeName === 'CANVAS') {
        ...
        if (model) {
          ...
          if (metaKey) {
            ...
            const normal = shiftKey
              ? int.face.normal.applyQuaternion(q)
              : undefined
          ...
          h.div({ class: 'image-import' }, [
            ...
            h.div([
              h.button({
                _: 'import',
                onclick() {
                  let { inv_image, inv_alpha, img_border, img_blur, img_base } =
                    api.modal.bound
                  load_files_opt(files, {
  ...
  function space_load(data) {
    if (data && data.length && (data = data.flat()).length)
      mesh.api.group
        .new(data.map((el) => new mesh.model(el)))
        .promote()
        .focus()
  }
