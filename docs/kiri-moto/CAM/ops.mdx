...
The trace operation supports continuous z-ramping for closed paths:
- **Continuous**: Linearly interpolates the Z depth around the path's perimeter as the tool travels continuously along the loop, avoiding discrete plunge step-downs between passes. If step down is set to `0`, a single continuous sweep from top Z to target bottom Z is generated. Open paths remain unaffected.
- **Flat Top**: When continuous mode is active, optionally executes a flat pass at top Z before ramping down.
- **Flat Bottom**: When continuous mode is active, optionally executes a flat pass at target bottom Z after completing the ramp passes (enabled by default to ensure a level floor).
