...
function init(mod) {
  ...
  function load_modules(root, force) {
    // load modules
    lastmod(`${dir}/${root}`) &&
      fs.readdirSync(`${dir}/${root}`).forEach((mdir) => {
        ...
        const isElectronMod = util.isfile(
          PATH.join(mod.dir, modpath, '.electron')
        )
...
const script = {
  kiri: ['@devices', 'kiri/ui', '&main/kiri', '&kiri/lang-en'],
...
function serveCode(req, res, code) {
  ...
  let cacheControl = code.nocache ? 'private, max-age=0' : 'public, max-age=600'
